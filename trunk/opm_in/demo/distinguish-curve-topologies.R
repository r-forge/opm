...
#' have tested by this the phenotypes of four biological specimens, e.g. bacterial
#' strains. For each strain one plate was utilized. Unfortunately, you do not
#' have replicate plates per bacterial strain.
#' 
#' ### The following question then arises:
#' * If four curves look fairly similar, how can we asses some statistical 
#' information whether they could be the same or be rather significantly 
#' different?
#' 
#' In order to asses this, we make use of bootstrapped curve parameters, which
#' allow to determine a 95 % confidence intervall
#' 
#' The data set `wittmann_et_al` contains GenIII data for numerous strains of
#' the bacterial species *Achromobacter xylosoxidans* (see the respective
#' publication).
#' 
#' 
...
#' * see [to_metadata()](http://www.goeker.org/opm/opm_doc/manual/collect_template.html) for details

dim(wittmann_small)
to_metadata(wittmann_small[, , ])

#' ### Plot the raw kinetic values for well G07 (D-Malic Acid) from the GenIII plate
#' * see [xy_plot()](http://www.goeker.org/opm/opm_doc/manual/xy_plot.html) for details
#' 
...
#' 
#' ### Do strains differ in any of the aggregated curve parameters A, AUC, mu, or lambda?
#' * To answer this, we make use of the 95% confidence intervalls obtained 
...
#' * the aggregated data contain 95% confidence intervalls from bootstrapping
#' * see [aggregated()]
...
aggregated(subset(wittmann_small[, ,"G07"], list(strain = "LMG 7051")), full = TRUE)

#' ### Plot confidence interval for the maximum height value (A)
#' * see [ci_plot()](http://www.goeker.org/opm/opm_doc/manual/ci_plot.html) for details
...
#' maximum height (A)
#' * similarly, also strains "LMG 7051" and "CCUG 48135" can not be distinguished
#' by A
...
#' 
#' ### Plot confidence interval for the steepnes of the slope value (mu) 
#' * see [ci_plot()](http://www.goeker.org/opm/opm_doc/manual/ci_plot.html) for details
