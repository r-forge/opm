...
y <- extract(x, as.groups = TRUE,  norm.per = c("column"), norm.by = 10,
  subtract = TRUE)
...
\rcscom{Is there anything wrong with the normalisation using only the value of one well (here "A10")?} 
