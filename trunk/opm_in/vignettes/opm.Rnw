...
{\color{red}In addition to 95\% confidence plots and enhanced heat-map graphics for visual comparisons of the estimated curve parameters, the package includes functionality for user defined simultanous multiple comparisons of group means.}
...
%\tableofcontents
%\newpage 
%% Please do NOT include the following line again, as it yields errors under Linux
%\SweaveOpts{concordance=TRUE}
...
{\color{red}Since most addressed experimental questions desire to statistically compare not only single curves, but distinct groups of curves, the package provides fuctionality for performance of simultaneous multiple comparisons of group means \citep{bretz}. Featured by methodology for highly flexible definition of groups using provided metadata, the user is enabled to compute contrast tests for individually defined sets of mean comparisons \citep{hsu}.}     
For further specific graphical or statistical analysis according to the users'needs, the \pkg{opm} package organises and maintains the data such that any additional data exploration using other packages in the \proglang{R} environment are easily applicable.

The work flow described below includes (i) the input of raw kinetic data and integration of corresponding metadata, (ii) conversion into suitable storage formats, (iii) the computation of a set of four parameters sufficient for comprehensively describing the curves' shape (aggregated data), (iv) manipulating and querying the constructed objects,{\color{red}(v) visualizing both raw kinetics and aggregated data, (vi) statistical comparison of group means, (vii) discretization of the curve parameters and corresponding export-methods and (viii) additional information about globals setting of the functionalities.}
...
{\color{red} TODO Lea: Point also on Fig. 2. Explain the right part of the figure.}
...
{\color{red}\subsection[Batch conversion of many files]{Batch conversion of many files}\label{methods batch}
...
{\color{red} TODO: Benjamin's approach(es) should be first mentioned here}
...
{\color{red} TOOD: add picture explaining these paramaters?}
...
{\color{red} TODO Lea: Explanation of Figure 3.}
...
{\color{red}\subsection[Plotting functions]{Plotting functions}\label{methods plotting}
...
{\color{red}\subsubsection[Plotting the aggregated data]{Plotting the aggregated data}\label{methods plotting-aggregated}}
...
{\color{red}
Since an PM-experiment usually contains sets of plates, which might not be measured all in the same experimental run, it is important to consider possible systematic variations and control for those. In terms of an PM-experiment the purpose of such a normalization is to minimize systematic variations in the aggregated curve parameters, so that biological differences can be more easily distinguished, as well as to allow the comparison of parameters across plates processed in different experimental runs. The underlying ideas are mainly derived from DNA-microarray experiments for measuring gene expression levels \citep{quackenbush}. 

With \code{extract()} the user is enabled not only to select certain aggregated or discetized values into common matrix- or dataframe-formats, but also to apply normalization-strategies before computing point estimates and respective confidence intervals for individually defined experimental groups. Although this function is intended mainly as a helper function for \code{ci_plot()}, it can be quite useful for specific normalization purposes, for example when data were derived before and after servicing the Omnilog\textregistered \ facility resulting in shifted results by a fixed value.}
...
{\color{red}\subsection[Statistical comparisons of group means]{Statistical comparisons of group means}

Since most addressed experimental questions of an PM-approach desire to statistically compare not only single curves, but distinct groups of curves, the analysis of a PM-data results in simultaneous inference across multiple questions \citep{hothorn}. To adress this issue the function \code{opm_mcp} provides fuctionality for performance of simultaneous multiple comparisons of group means by internally calling \code{glht} from \pkg{multcomp}. Using available metadata and/or the substrate-names the user is enabled to define groups of interest, set up a model of choice and perform multiple comparison of group means on individually specified contrasts \citep{bretz, hsu}. As not differentially specified, comparisons for the different curve parameters are performed separately.

At this point, the power and flexibility of simultaneous multiple comparison procedures will be figured out and the user shall be encouraged to apply contrast tests on individually designed sets of mean comparison rather than to employ probably better-known classical ANOVA-based approaches followed by F-tests. It has to be pointed out, that, generally, F-tests provide not more than global information about main effects and interaction effects. That is nothing more than a significant result gives evidence for a difference in means among any of the considered treatments. In the framework of PM-results an significant F-test for example the effect of the substrate would indicate that at least two of the substrates cause different respiration. Considering that each experiment encounters at least 96 different substrates (up to 2,000) this information is obviously nearly useless. Moreover, F-tests do not provide information about effect sizes neither is it possible to easily adress comparisons of particular interest \citep{schaarschmidt}.

From our point of view, the very most underlying experimental question in PM-experiments can be best expressed as a set of particular mean comparisons, resulting in a multiple comparison problem \citep{hochberg}. However, testing an increasing number of hypothesis with the number of true hypotheses unknown, the probability of at least one wrong testing decision increases. Thus the so-called family-wise error-rate (FWER), which is essentially the probability of at least one false rejection among all the null hypotheses, needs to be controlled \citep{tukey}.
The here internally employed functionality from the package \pkg{multcomp} provides solutions for all listed difficulties, since it allows to test user defined set of contrasts based on a broad range of model-types while internally controling the FWER.  

% TODO: describe here, why multiple comparisons of means, why not ANOVA, why multcomp and nothing else?
% TODO Lea Introduce functionality of \code{opm\_mcp()} and \code{extract()}; explain the implementation of how factors
% are merged to produce a pseudo-one-way-layout \citep{schaarschmidt}.
The function internally reshapes the data into a 'flat' data frame containing one column for the chosen parameter-value, one column for the well-name and optionally additional columns for the selected metadata. For performance of the testing procedure, a model has to be stated thereby specifying the factor's levels which determine the grouping \citep{searle, hothorn}.
}

{\color{green}Vorschlaege johannes:
(3) Wir sollten das prinzipielle Vorgehen der multcomp-Methode erlaeutern (auch wenn es da schon haarig und anspruchsvoll wird), da ist aber die Art und Weise, wie du mir das unten beschrieben hast, schon sehr hilfreich.}
...
TODO:write description and what's about global settings?}
...
If one uses \code{read\_opm(..., convert = "grp")}, a named list is created with, as each list element, one \proglang{OPM} or \proglang{OPMS} object per plate type.
The plate types could then easily be accessed \textit{via} the names of the list.
...
{\color{red}\subsection[Batch conversion of many files]{Batch conversion of many files}\label{application batch}
...
%% TODO: whats about example for OPMS-object including metadata?
}
...
TODO Lea: here the application of \code{opm\_mcp()} with example and output-graphics.
reshaping the data
<<results=hide, width=8>>=
data(vaas_4)
...
Explain contrasts, refer to \code{contrMat() from \pkg{multcomp}}
...
Explain Confidence intervals for differences of group means, point out the difference to the 95\%CI of the curve-parameter estimators.
