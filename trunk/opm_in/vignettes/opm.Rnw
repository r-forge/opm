...
and note that the \code{phylo\_data()} has a ``html.args'' argument. Textual \proglang{HTML} output supports most of the formatting instructions  for the output of \proglang{HTML} tables described below (see \ref{application discrete-table}). Note particularly how formatting \textit{via} a \proglang{CSS} file works.
...
A practical problem is that the resulting \proglang{HTML} is linked to its \proglang{CSS} file with a fixed path.
The formatting would thus get lost once the \proglang{HTML} was copied to another system, without a warning.
So users might want to copy the predefined \proglang{CSS} file to the working directory, link the generated \proglang{HTML}
...
%% TODO: describe copying of the CSS file
