...
Especially for data situations, where the groups are defined by more than one metadata-information (i.e. more than one metadata-column) the evaluation of differences of treatment means may result in quite complex models. 
...
Technically, this necessitates to merge all defining metadata factor-variables in one factor. In Section~\ref{application metadata} an example using \code{interaction()} for factor-merging is given and the computation of multiple comparisons using a \textit{cell-means-model} is shown in Section~\ref{application group-means}
...
%Lea, please have a look into this.
%}
...
\rcsadd{In order to prepare the metadata for being able to set up a \textit{cell-means-model}(see Section~\ref{methods group-means} and ~ref{application group-means} for further details), factors used for defining groups of interest have to be merged. 
This might be done at the initial step when setting up the metadata by using \code{interaction()} for concatenating existing levels of choosen factor-variables.}

<<results=hide, width=10>>=
metadata.example$new.factor <- interaction(metadata.example$Colou, 
...
Using the more flexible framework of employing formulas and expressions for modification of  metadata together with \code{paste()} eases here the subsequently manipulation of metadata.

<<results=hide, width=8>>=
metadata(example.opm) <-  new.Meta ~ paste(`Colony colour`, Integer, sep = ".")
metadata(example.opm)
@
...
metadata(vaas.G06) <- new.MetaI ~ interaction(Strain, Experiment)
# all types of metadata 
metadata(vaas.G06) <- new.Meta ~ paste(Strain, Experiment, sep = ".")
@

With the newly generated metadata-variable \code{new.Meta} comprehending \code{Species} and \code{Strain}, the multiple comparison procedure is straightforwardly processable:

<<results=hide, widht=8>>=
vaas5.mcp <- opm_mcp(vaas.G06, model = ~ new.Meta, 
  linfct = mcp(new.Meta = "Dunnett"))
@

The corresponding visualisation of the CI is shown in Figure~\ref{fig:opm_mcp_CI_plot_cell-means}.
...
\caption{
\label{fig:opm_mcp_CI_plot_cell-means}
...
Finally the user has the full flexibility of defining groups of interest and compare them in sophisticated manner not only using single group means, but also averages of subgroups versus single groups or averages of other subgroups.
When building a contrast-matrix, keep in mind that the levels of model-defining factor frame the columns in the same order as they occur in the underlying dataframe.
Further, no restriction apply on the chosen numbers for coefficients except, that the sum of the coefficients of one contrast needs to be zero. 

<<results=hide, width=8>>=
# provide data for multiple comparison
data <- opm_mcp(vaas.G06, model = ~ new.Meta, 
  linfct = mcp(new.Meta = "Dunnett"), output = "data")

# check level-names and order of the group defining factor
levels(data$new.Meta)
...
colnames(contr) <- levels(data$new.Meta) 
contr

# compute multiple-comparison based on the defined contrast-matrix
vaas6.mcp <- opm_mcp(vaas.G06, model = ~ new.Meta, m.type = "lm",
  linfct = mcp(new.Meta = contr))
@
