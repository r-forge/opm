...
To demonstrate the use of \code{opm\_mcp()}, the function for conducting multiple comparisons of group means directly on \proglang{OPMS} objects \citep{Vaas2013}, we compare four distinct strains, each of which represented by ten replicates of GEN-III microplate measurements.
...
When performing PM-experiments, usually the well-wise comparison of strains, treatments, etc. for complete plates is needed and can be performed easily by using the specially designed \code{linfct}-argument \code{Pairs}.
...
Note that \code{Well} has to be part of the model and joined with at least one other metadata-factor in order to determine the groups to be compared. The given example computes all 96 tests for one OPMS object comparing the species on each well.
