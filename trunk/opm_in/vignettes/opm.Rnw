...
Four descriptive curve parameters are estimated, which is explained in detail in Section~\ref{methods aggregating}, whereas example code for curve parameter aggregation is included in Section~\ref{application aggregating}.

The user may now be interested to either query or subset the diverse set of data stored in \proglang{OPM(A)(D)(S)} objects. 
The underlying principles are described in Section~\ref{methods manipulation}, whereas example code for object management is included in Section~\ref{application manipulation}.

The raw kinetic data can be plotted either as levelplots or as typical curves in form of XY plots. The aggregated curve parameters can be plotted either as confidence-interval plots (comparison of single curves), radial plots (comparison of single/few wells across several plates) or heatmaps (comparison of all wells across several plates). 
The underlying principles are described in Section~\ref{methods plotting}, whereas example code for plotting is included in Section~\ref{application plotting}.

In order to then statistically compare aggregated curve parameters, functionalities for the multiple comparison of groups means are implemented. 
This allows to test statistical hypothesis involving groups of numerous plates (as defined, e.g., \textit{via} metadata entries) and/or groups of numerous wells (as defined, e.g.,  \textit{via} KEGG or COG groups). 
\rcscom{is that possible?, to define groups via KEGG or COG groups?}
...
The aggregated data can be discretized and exported for phylogenetic analysis in specialized phylogeny software such as \proglang{PAUP*}, \proglang{RAxML} or \proglang{TNT}. The principle is outlined in Section~\ref{methods discrete-multistate}.
Also, a typical application of the OmniLog\textregistered \ technology is to classify reactions as being either positive, negative or weak (ambiguous). 
In order to not rely on artificial user-defined threshold values, k-means partitioning algorithms have been implemented to obtain non-biased discretization into these categories based on the data (maximum value A) itself. The  principle is described in Section~\ref{methods discrete-binary}. Example code for these discretization applications, including the export of discretization results as  tabular \proglang{HTML} or text generation suitable for publication, is included in Section~\ref{application discrete-phylogeny}.
...
Finally, it is possible to modify settings that have an effect on multiple functions and/or on frequently used arguments, see Section~\ref{application settings}.


\label{methods manipulation}


\rcscom{
All subsections are mentioned and linked. The section \proglang{Batch conversion of many files} is not yet addressed (see TODO entry for Lea).
}
...
\rcscom{
Lea, I am not sure whether this is the best position for this subsection.
Apparently batch conversion involves a lot of actions that are only explained later on.
But if this was an argument, then this subsection should better be placed at the very end of the whole section.
}
