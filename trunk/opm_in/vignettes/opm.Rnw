...
But the OmniLog\textregistered \ software can batch-export one plate per CSV file, and \pkg{opm} can split multi-plate CSV into files that can be input by the package.
...
At this point, it is necessary to highlight the power and flexibility of simultaneous multiple comparison procedures and encourage the user to apply contrast tests on individually designed sets of mean comparison rather than to employ the probably more popular classical ANOVA-based performing F-tests.
...
Users of multiple-comparison procedures, especially simultaneous multiple contrast-tests as applied here, are encouraged to have a look at the book of \citep{hochberg} and \citep{hsu}. 
In order to evaluate the differences of treatment means where the groups are defined by more than one metadata-column and thus may result in quite complex models, we suggest to set up \textit{cell-means-models} (also known as \textit{pseudo-one-way-layouts} as discussed in \citep{schaarschmidt}). 
In this approach estimators for treatment and variance are derived from a model with all treatment (here:metadata) combined in a single factor. 
...
This optionally includes aggregating the raw data by estimating curve parameters (Section~\ref{application aggregating}), discretizing these parameters (Section~\ref{application discrete-text}) and integrating metadata (Section~\ref{application discrete-text}). 
...
It is possible to map other types of entries such as numeric vectors by requesting their coercion to the character type; see the \code{?map\_metadata} for details.
...
The argument \code{m.type} specifies the model type to be used for fitting, e.g., a linear model (\code{lm}), a generalized linar model (\code{glm}), or an analysis of variance model (\code{aov}).
...
This results in a set of six two sided pairwise comparisons amoung all the four strain-means (all possible pairs).
...
A numerical output of the results is available with the following:

<<results=hide, width=8>>=
sumx <- summary(xx)
sumx$model$call <- NULL
sumx
@

The interpretation of confidence intervals for differences of means is somewhat different from the interpretation of the CIs for the point estimators for curve parameters as discussed in Section~\ref{application plotting-aggregated}.
...
In the here shonw example all group means are significantly different from each other, except for the comparison of strains DSM30083T and DSM1707 (Figure~\ref{fig:ci_plot_mcp}).

The last comparison showing the difference of mean of curve parameter A values of strain DSM 30083T minus the mean of A values of strain DSM18039 results 26.615 units. 
Therefore, the point estimator of this difference of specific group-means is plotted at position 26.615 on the x-axis. 
That is, in mean A-values for Strain DSM30083T is 26.615 units larger than the mean A-value of strain DSM18039 (for the detailed numeric outcome see the estimate results provided by \code{sumx)} or \code{confint(xx)}).
...
The main steps the function \code{opm\_mcp()} internally proceeds are a reshaping of the data into a flat-file formatted dataframe and the application of \code{glhp} from \pkg{multcomp}.
The reshaping can be viewed, when the argument \code{do.mcp = FALSE}:
...
The function \code{contrMat()} from \pkg{multcomp} provides an overview about the predefined contrast-types, which can be used in the \code{opm\_mcp()}-argument \code{mcp.def}.
...
Using the example from above (see Figure~\ref{fig:ci_plot_mcp}), a brief illustration of a resulting contrast matrix is given.
``Tukey''-type contrasts define to compare all group-against all others in the dataset. The underlying contrast-matrix can be viewed by typing  
<<width=8>>=
summary(xx)$linfct
@

Analogously, the user is free to set up contrast-matrices containing the comparisons of interest.
However, it must be set up a model first, which defines which factor determines the groups and thus the possibilities for comparisons.
For matters of convenience, we want to compare the overall-performance of the tested organisms on the first four wells A01 to A04. 
...
Using the \code{opm_mcp()}-function, the resulting code ends in:
<<results=hide, width=8>>=
x <- opm_mcp(vaas_4[, , 1:4], model = ~ Well, m.type = "lm", mcp.def = contr)
@

As mentioned above, visualization of the results is available by using the \code{plot()}-method for \code{glht}-objects:
...
Note, that the \code{model}-statement defines, which group-means are available for comparisons. 
In the following example \code{Species} contains only two levels (``Pseudomonas aeruginosa'' and ``Escherichia coli''). 
Thus, irrespectively of the stated contrast-type, only one comparison is possible.
