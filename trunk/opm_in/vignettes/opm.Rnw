...
In brief, four bacterial strains were measured on  GEN-III plates (AES Chemunex BLG 1030) in the PM modus.
...
Descriptive curve parameters from the kinetic raw data can be calculated and included in \proglang{OPM} and \proglang{OPMS} objects using the function \code{do\_aggr()} (see Figure~\ref{fig:work_flow}).
Curve parameters can be extracted using a spline-based fitting procedure implemented in \pkg{opm}. Three different modelling alternatives for the splines exist:
...
\rcsmark{
We should add own picture explaining these parameters.
}
...
\rcsmark{Update needed, should be merged with old Figure 1.}
...
\rcsadd{Normalization indicates here the subtraction of the mean of each plate from each of its values (over all wells of that plate), either per row or per column. Alternatively, all values can be divided by the mean of the value. The details can be set in the respective arguments of \code{extract()}.}
...
\rcscom{
Style in the next paragraphs seems a bit too colloquial.
At lot of ``let us'', for instance.
Other parts repeat a lot of text from the method section.
The variable names should be in the style used above in this vignette.
Please revise.
Also, please consider the TODO entries in the code for opm\_mcp().
}
