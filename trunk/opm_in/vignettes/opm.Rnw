...
Since the complete code of this Vignette comes with the R-package in a file called \textit{opm.R} (can usually be found under \path{...\R\R-X.X.X\library\opm\doc}), application of the here given examples and demonstrations are straightforwardly processable. 
...
<<label=include.metadata, width=8>>=
...
<<label=to.metadata, results=hide>>=
metadata(example.opm)
...
<<label=include.metadata, results=hide, width=8>>=
example.opm <- include_metadata(example.opm, md = metadata.example)
metadata(example.opm)
...
<<label=norm.wellA10, results=hide, fig=FALSE, width=10>>=
