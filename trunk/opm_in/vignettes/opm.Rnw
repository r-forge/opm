...
% Avoid the term "functionality". It had been used way too frequently.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\Plaintitle{opm: An R package for analysing OmniLog Phenotype MicroArray Data}

% short title, header on every 2nd page
\Shorttitle{Phenotype MicroArray Data (\today)}
...
In addition to 95\% confidence plots and enhanced heat-map graphics for visual comparisons of the estimated curve parameters, the package includes customized methods for user-defined simultaneous multiple comparisons of group means.
...
Export and import in the \proglang{YAML} (or \proglang{JSON}) markup language (or as character-separated values) facilitates the data exchange among labs.
...
This is the tutorial of \pkg{opm} in the version of \today{}.
...
The interested user would nevertheless find them in Section~\ref{scientific introduction}, including references for important methods.
...
The system is applicable, in principle, to each kind of cultivated cells and also to environmental probes, even though some kinds of cells, such as those from plant cell cultures, well cause a reduction of the dye but are too large to be handled in the 96-well layout \citep{ijms140715141}.
...
Based on a previous study \citep{vaas} the here presented \pkg{opm} package \citep{Vaas2013} offers functionalities for a fast, robust and comprehensive evaluation of PM respiration kinetics suitable for a wide range of experimental questions.
...
As \pkg{opm} is also able to generate \proglang{R} matrices and data frames, output in CSV (character-separated values) is also easy.
...
Since most addressed experimental questions require to statistically compare not only single curves, but distinct groups of curves, the package provides adapted methods for performing simultaneous multiple comparisons of group means \citep{bretz}.
...
This includes storage of the \pkg{opm} data in the \proglang{YAML} (or \proglang{JSON} or \proglang{CSV}) format, as detailed in Section~\ref{methods batch}.
...
As of version 0.4-0, \pkg{opm} also supports the input of MicroStation\textregistered \ CSV files (frequently used in conjunction with EcoPlate\textregistered \ assay for microbial community analysis) \citep{Vaas2013}.
...
To process and store huge numbers of raw data files, the function \code{batch\_opm()} reads all OmniLog\textregistered \ CSV files (or \proglang{YAML} or \proglang{JSON} files previously generated with \pkg{opm}) within a given list of files and/or directories and converts them to \pkg{opm} \proglang{YAML} (or \proglang{JSON} or \proglang{CSV}) format.
...
Three different modelling alternatives for the splines exist \citep{Vaas2013}:
...
The parameters can nevertheless be robustly estimated from deviating curves, particularly \textit{via} spline fits \citep{vaas, Vaas2013}.
}
\end{figure}
...
Thus, the function enables the user to compare the curve data in a customized and useful arrangement \citep{vaas, Vaas2013}.
...
The user can obtain an overview in a compacted design \citep{vaas, Vaas2013}.
...
To address this issue the function \code{opm\_mcp()} performs simultaneous multiple comparisons of group means by internally calling \code{glht()} from the \pkg{multcomp} package \citep{hothorn} but providing an easier interface for it, specifically adapted to the typical objects used within \pkg{opm}.
...
The here internally employed functions from the package \pkg{multcomp} provides solutions for all listed difficulties, since it allows for testing a user-defined set of contrasts based on a broad range of model types while internally controlling the family-wise error-rate.  
...
Technically, this requires the merging of several defining metadata variables into a single one.
This can be done by creating new metadata entries from given ones and storing them back in an \proglang{OPM} or \proglang{OPMS} object.
An according example is given in Section~\ref{application metadata}.
Alternatively, merging can be done when selecting metadata for creating data frames.
The computation of multiple comparisons using a \textit{cell-means model} is shown in Section~\ref{application group-means}.
...
The \pkg{opm} package includes data transformations (implemented in the \code{discrete()} methods) for coding continuous characters by assigning them to a given number of equal-width categories within a given range.
...
<<label=tidy-split-files-up, results=hide>>=
rm(multi.plate.file)
...
<<label=read-many-plates, results=hide>>=
...
<<label= batch-opm-demo, results=hide>>=
...
<<label=batch-opm-real, results=hide>>=
...
<<label=opm-scripts, results=hide>>=
opm_files("scripts")
@
...
<<label=collect-template-with-file>>=
...
<<label=to-metadata-convert, results=hide>>=
metadata(example.opm)
...
<<label=map-metadata-with-formula, results=hide>>=
metadata(example.opm)
...
<<label=clean-metadata-with-formula, results=hide>>=
...
<<label=OPMS-clean-metadata-with-formula, results=hide>>=
...
<<label=metadata-expression, results=hide>>=
metadata(example.opm) <- to_metadata(csv_data(example.opm))[, 
...
<<label=empty-metadata, results=hide>>=
...
<<label=assign-metadata, results=hide>>=
...
<<label=map-included-metadata, results=hide>>=
example.opm <- include_metadata(example.opm, md = metadata.example)
metadata(example.opm)
...
<<label=metadata-chars, results=hide>>=
example.opm <- include_metadata(example.opm, md = metadata.example)
...
<<label=map-metadata-names-with-vector, results=hide>>=
...
<<label=map-metadata-values-with-vector, results=hide>>=
...
<<label=merge-metadata-by-hand, results=hide>>=
...
<<label=merge-metadata-with-formula, results=hide>>=
...
<<label=load-example-data, results=hide>>=
data(vaas_1)
vaas_1
@
...
One can also specify different spline fitting methods using \code{method = "spline"}, which is the recommended setting (for historical reasons, it is not the default) \citep{Vaas2013}.
...
<<label=data-vaas-small, results=hide>>=
data("vaas_et_al", package = "opmdata")
...
<<label=query-metadata-k-one-key, results=hide>>=
...
<<label=query-metadata-k-two-keys, results=hide>>=
...
<<label=query-metadata-q-keys-with-values, results=hide>>=
...
<<label=query-metadata-q-alternative-values, results=hide>>=
...
<<label=query-metadata-and-select, results=hide>>=
...
A peculiarity of \code{extract()} is that formulas can be used to trigger the joining of selected metadata entries (converted to data-frame columns) into new ones, using the pseudo-function \code{J()} within the formula.
For instance, the following code would create a new entry called ``Species.Strain'':

<<label=extract-with-joining, results=hide>>=
vaas.mu <- extract(vaas_et_al, dataframe = TRUE,
  as.labels = ~ J(Species, Strain), subset = "mu")
@

This is applied by \code{opm\_mcp()}, see Section~\ref{application group-means}.
The behaviour during joining of factors is modified using \code{opm\_opt(comb.key.join = ...)} and \code{opm\_opt(comb.value.join = ...)}.
...
<<label=xy-plot-dummy, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=xy-plot-figure, fig=TRUE, echo=FALSE, width=18, height=12>>=
...
<<label=xy-plot-partial-dummy, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=xy-plot-partial-figure, fig=TRUE, echo=FALSE, width=9, height=6>>=
...
<<label=level-plot-dummy, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=level-plot-figure, fig=TRUE, echo=FALSE, width=18, height=12>>=
...
<<label=heat-map-dummy, results=hide, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=heat-map-figure, fig=TRUE, echo=FALSE, results=hide, width=9, height=6>>=
...
<<label=radial-plot-dummy, results=hide, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=radial-plot-figure, fig=TRUE, echo=FALSE, results=hide, width=9, height=6>>=
radial_plot(vaas_4[, , c(1:5, 10)], as.labels = list("Species", "Strain"),
  x = 150, y = 200)
@
}
...
<<label=ci-plot-dummy, results=hide, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=ci-plot-figure, fig=TRUE, echo=FALSE, results=hide, width=9, height=6>>=
...
<<label=extract-dataframe, results=hide>>=
x <- extract(vaas_et_al, as.labels = list("Species", "Strain"),
  dataframe = TRUE)
@

For a better understanding of the following secondary applications of \code{extract()} it is highly recommended to take a look at the results from plotting the data with \code{ci\_plot()} and also at structure of the created data frames.
...
<<label=ci-plot-of-groups-dummy, eval=FALSE>>=
# without normalisation
ci_plot(extract(x, as.groups = TRUE,  norm.per = "none")[, c(1:7, 13)],
  legend.field = NULL, x = 350, y = 0)
@

\begin{figure}
\scalebox{1.225}{
<<label=ci-plot-of-groups-figure, fig=TRUE, echo=FALSE, width=9, height=8>>=
ci_plot(extract(x, as.groups = TRUE,  norm.per = "none")[, c(1:7, 13)],
  legend.field = NULL, x = 350, y = 0)
@
}
...
The following code would first normalize with the plate means, then with the well means:

<<label=ci-plot-normalisation, eval=FALSE>>=
ci_plot(extract(x, as.groups = TRUE,  norm.per = "row")[, c(1:7, 13)],
  legend.field = NULL, x = 150, y = 0)
ci_plot(extract(x, as.groups = TRUE,  norm.per = "column")[, c(1:7, 13)],
  legend.field = NULL, x = 150, y = 0)
@
...
One could normalize by subtracting the means of well A10 only as follows:

<<label=ci-plot-norm-well-A10-dummy, eval=FALSE>>=
ci_plot(extract(x, as.groups = TRUE,  norm.per = "row",
...
\begin{figure}
\scalebox{1.225}{
<<label=ci-plot-norm-well-A10-figure, fig=TRUE, echo=FALSE, width=9, height=8>>=
ci_plot(extract(x, as.groups = TRUE,  norm.per = "row",
  norm.by = 10, subtract = TRUE)[, c(1:7, 13)],
  legend.field = NULL, x = 0, y = 0)
@
...
The \code{opm\_mcp()} function allows to test for differences in the means of multiple groups directly on \proglang{OPMS} objects, obtaining the factors for the grouping structure from stored metadata. 
In the following, the general concept and the structure of the function will be explained using different examples for groups that can be defined within wells, across wells, or across metadata-defined groups. 
The different examples will be addressed in the subsections shown below.
Also, an explanation of how the graphical and numerical output of results is to be interpreted is given.


\subsubsection[Tukey type of comparison: all-against-all]{Tukey type of comparison: all-against-all}\label{Tukey-type comparison}

This paragraph addresses the comparison of a single well type across different plates organised into multiple groups.
We compare four distinct strains, each of which being represented by ten replicates of GEN-III microplate measurements. 
The experimental question to be addressed relates to a single well: ``Do these four bacteria differ in the mean value of curve parameter A on well G06?'' (see Figure~\ref{fig:xy_plot_mcp}).
This type of comparison is termed "Tukey"-type contrasts (all-against-all) because each strain is compared to each other.

The necessary data subset can be taken from the first biological replicate included in \code{vaas\_et\_al}:

<<label=data-vaas-subset, results=hide>>=
vaas.G06 <- subset(vaas_et_al[, , "G06"], list(Experiment = "First replicate"))
@

The resulting dataset of four strains, each represented by the ten replicates, is shown in Figure~\ref{fig:xy_plot_mcp}. 

<<label=plot-vaas-subset-dummy, eval=FALSE>>=
xy_plot(vaas.G06, include = ~ Strain, neg.ctrl = FALSE, space = "right")
@

\begin{figure}
\scalebox{1.0}{
<<label=plot-vaas-subset-figure, fig=TRUE, echo=FALSE, width=9, height=6>>=
xy_plot(vaas.G06, include = ~ Strain, neg.ctrl = FALSE, space = "right")
@
}
\caption{
...
As explained in section \ref{methods group-means}, the \code{opm\_mcp()} methods internally reshape the data into a data frame containing one column for the chosen parameter, one column for the well (substrate) name, another column for the values itself and optionally additional columns for the selected metadata. 
The argument \code{output = "data"} shows the data frame created by \code{opm\_mcp()}. 
The code below summarizes the respective data frame, which contains the A values (maximum height) of the well G06 (a-Keto-Glutaric Acid) from four strains of each 10 plates:

<<label=opm-mcp-data>>=
summary(opm_mcp(vaas.G06, model = ~ Strain, m.type = "aov",
  linfct = c(Tukey = 1), output = "data", full = FALSE))
@

For performing the testing procedure, a model has to be stated that specifies the factor levels that determine the grouping \citep{searle, hothorn}.
The groups to be compared (and here to be selected from the metadata beforehand) are defined by the argument \code{model}.
...
In principle, the contrast matrix defines from which groups the means should be compared and how.
In our example a Tukey-type contrast matrix is used (a more detailed explanation of contrast matrices, their structure and features, is given in the help page of \code{multcomp::contrMat()}).
The test results in a set of six two-sided pairwise comparisons between all four strain means (all possible pairs).
The results of the comparison is written into an object termed, e.g., \code{vaas.G06.mcp}:

<<label=opm-mcp-principle, results=hide>>=
vaas.G06.mcp <- opm_mcp(vaas.G06, model = ~ Strain, m.type = "aov",
  linfct = c(Tukey = 1))
@

The here used kind of \code{linfct} argument, a numeric vector, refers to the positions of the variables within \code{model} and here, by using $1$ the first and only term, ``Strain'', is selected.
The names of such a \code{linfct} object indicate the type of contrast used, here ``Tukey''.
Per default \code{opm_opt("contrast.type")} would be inserted, if names were missing.


Note that the structure of the arguments set by \code{model} and by \code{linfct} may become more complex if several metadata entries are involved in the testing.
The user might therefore wish to recheck the way how \code{model} and by \code{linfct} will actually perform during the execution of the statistical test.

The correct usage of the argument \code{model} can be controlled by:

<<label=plot-opm-mcp-linfct-dummy>>=
opm_mcp(vaas.G06, model = ~ Strain, m.type = "aov",
  linfct = c(Tukey = 1), output = "model")
@

The correct usage of the argument \code{linfct} can be controlled by:

<<label=opm.mcp.linfct>>=
opm_mcp(vaas.G06, model = ~ Strain, m.type = "aov",
  linfct = c(Tukey = 1), output = "linfct")
@


For the computation of the tests itself, \code{opm\_mcp()} uses \code{glht()} from the package \pkg{multcomp} and returns an object of class \code{glht}.
...
<<label=plot-opm-mcp-linfct-dummy, eval=FALSE>>=
...
\begin{figure}
\scalebox{1.225}{
<<label=plot-opm-mcp-linfct-figure, fig=TRUE, echo=FALSE, width=8, height=3>>=
library(multcomp)
...
<<label=summary-opm-mcp-Tukey>>=
mcp.summary <- summary(vaas.G06.mcp)
...
The point estimator for differences of means represents the computed difference of the considered group means and, analogously, the size of the CI indicates the reliability of this difference.
If the 95\% CI for differences of means includes zero (dashed vertical lines in Figure~\ref{fig:ci_plot_mcp}) there is no significant difference between the group means.
Conversely, if zero is not included, a statistically significant difference is indicated.
Furthermore, the more distant the 95\% CI is from zero, the larger the biological effect size, i.e. the real difference between the group means. 

In the here shown example all group means of the curve parameter A are statistically significant different from each other (p < 0.001), except for the comparison of strains DSM 30083\textsuperscript{T} and DSM 1707 (Figure~\ref{fig:ci_plot_mcp}).

For an explanation of the graphical representation of the CIs, consider the last comparison in Figure~\ref{fig:ci_plot_mcp}.
The comparison of the mean A value from strain DSM 30083\textsuperscript{T} minus the mean A value of strain DSM 18039 results in 26.615 units as the point estimator of this difference which is plotted accordingly on the x-axis.
...
(The detailed numeric outcome would be obtained using \code{mcp.summary()} or \code{confint(vaas.G06.mcp)}).
Additionally, each point estimator for the difference of means comes with a 95\% CI providing information about the statistical significance of the test, the effect size and the variability of the mean differences.
...
But their numerical values are not correlated with effect sizes. % unclear
The position of the 95\% CI indicates that the difference between strains DSM 18039 and 429SC1 (i.e, the effect size in the second comparison in Figure~\ref{fig:ci_plot_mcp}) is much larger than between strains DSM 30083\textsuperscript{T} and 429SC1 (smaller effect size in the third comparison).
For a meaningful biological interpretation of the results it is therefore highly recommended to consider the effect size rather than taking only the p values into account.


\subsubsection[Dunnett-type comparison of complete plates]{Dunnett-type of comparison: one-against-all}\label{comp-plates-dunnett}

This paragraph describes another type of comparison of the means of multiple groups, which is the comparison of  a single, selected well against all other wells of the same plate.
This type of comparison is termed ``Dunnett''-type contrasts (one-against-all). 
In the example below, the groups are defined by different wells and not by different plates.
The reference well can be a negative or positive control or, for example, one substrate serving as a standard in a specific chemical group. 
The following data example is again taken from the first biological replicate included in \code{vaas\_et\_al}, but this time only the type-strain \textit{Escherichia coli}, measured in ten technical replicates, is selected.

<<label=vaas-e-coli, results=hide>>=
vaas.e.coli <- subset(vaas_et_al, 
  list(Experiment = "First replicate", Strain = "DSM30083T"))
@

The comparison of all wells against the negative control in A01 can now be performed by calling (for matter of convenience, we perform the tests only for the first ten wells): 

<<label=vaas-e-coli-opm-mcp, results=hide>>=
opm_mcp(vaas.e.coli[, , 1:10], output = "mcp", model = ~ Well, 
  linfct = c(`Dunnett_A01 (Negative Control)` = 1))
@

Please note a speciality of the code above: generally, the value for the \code{linfct} argument can be constructed by typing \code{Dunnett}, and, separated by any sign, e.g., underscore (\_), the level name which should serve as the reference group in the contrast set. 
This substantially eases the choice of the reference group.

A summary of the resulting data frame, with well A03 chosen to be the reference group, shows the A values from ten wells of all ten plates:

<<label=summary-opm-mcp-Dunnett>>=
summary(opm_mcp(vaas.e.coli[, , 1:10], output = "data", model = ~ Well,
  linfct = c(`Dunnett_A03` = 1), full = FALSE))
@


<<label=vaas-e-coli-mcp-Dunnett-special, results=hide>>=
mcp.A03 <- opm_mcp(vaas.e.coli[, , 1:10], output = "mcp", model = ~ Well,
             linfct = c(`Dunnett_A03` = 1), full = FALSE)
@

The resulting object is of class \code{glht} and can be plotted as usual using \code{plot()}.
Below is the summary (the output of the code is suppressed, but see the example above):

<<label=vaas-e-coli-mcp-Dunnett-special-summary, echo=FALSE>>=
options(width=200)
mcp.summary <- summary(mcp.A03)
mcp.summary$model$call <- NULL # avoid some unnecessary output
mcp.summary
@
...
\subsubsection[Pairs-type comparison of specific wells]{Pairs-type comparison of groups: pairwise comparisons as defined by specific combinations of metadata entries}\label{specific-well-Pairs}


This paragraph describes a more specific and hence complex design of the group structure by making a more distinct use of the stored metadata and its potential combinations. 
Assume data from two species, \emph{P. aeruginosa} and \emph{E. coli}, with each two plates (data set \code{vaas_4}). 
The data from wells A01, A02, A03, and H02 have been chosen, see below. 

<<label=opm-mcp-show-data-dummy, eval=FALSE>>=
opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ Species, m.type = "aov",
        linfct = c(Tukey = 1), output = "data", full = FALSE)
@


A combination of two species $\times$ four well types would yield eight different groups, which are each represented by two plates.
A Tukey-type comparison (all-against-all) would then result in 28 pairwise comparisons, whereas a Dunnett-type comparison (one-against-all) would result in seven pairwise comparisons.
However, assume the user is only interested in a specific subset of pairwise comparisons defined by questions such as ``For each well, is there a difference between the two species?''
This experimental question resulted in testing four statistical hypotheses, as there are only four pairwise combinations that fit this question, since for each of the four wells, the two species would be compared. 
Hence, it needs to be ensured that the adjustment for multiplicity (see section~\ref{methods group-means}) is done only for these four statistical hypotheses, despite the presence of eight groups in the data set. 

This can easily be performed by applying the specially designed \code{linfct} argument \code{Pairs}. 
First, the user needs to take care that \code{Well} has to be part of the model and is joined with at least one other factor extracted from the metadata, in this case with \code{Species}. 
This is achieved by the \code{J()} function (\code{model = ~ J(Well + Species)}), as shown below.


<<label=opm-mcp-linfct-Pairs-1a, eval=FALSE>>=
opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ J(Well + Species), m.type = "aov",
        linfct = c(Pairs = 1), output = "data", full = FALSE)
@

Note that the resulting \code{model} vector ``Well.Species'' contains eight levels, i.e. eight groups of each two plates.

<<label=opm-mcp-linfct-Pairs-1b, echo=FALSE>>=
options(width=200)
opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ J(Well, Species), m.type = "aov",
        linfct = c(Pairs = 1), output = "data", full = FALSE)
@

Second, the argument \code{linfct = c(Pairs = 1)} ensures that for all levels present in the first argument of \code{model = ~ J(Well, Species)}, i.e, \code{Well}-wise, all pairwise comparisons are performed among the different groups present (here: the two species \emph{P. aeruginosa} and \emph{E. coli}).

The results of this analysis is shown below.

<<label=opm-mcp-linfct-Pairs-1c, width=8>>=
y <- opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ J(Well, Species), 
             m.type = "aov", linfct = c(Pairs = 1))
@

\tiny
<<label=summary-opm-mcp_2 ,echo=FALSE>>=
options(width=200)
mcp.summary <- summary(y)
mcp.summary$model$call <- NULL # avoid some unnecessary output
mcp.summary
@
\normalsize
...
If models are more complex, e.g., if more than two metadata entries would be joined by the  \code{J()} function, the names of the metadata entries for which the pairwise-comparisons should be performed can be appended directly to the \code{Pairs} argument.
For instance in the example addressed above, by using \code{linfct = c(Pairs.Species = 1)} the metadata name \code{Species} can directly be addressed. 

Thus the result of 

<<label=opm-mcp-linfct-Pairs-2, results=hide>>=
y <- opm_mcp(vaas_4[, , c(1:3, 86)], model = ~ J(Well, Species), m.type = "aov",
...
is the pairwise all-against-all comparisons of the selected four wells \textit{\textbf{within each}} of the two species \textit{Escherichia coli} and \textit{Pseudomonas aeruginosa}.
This yields 2 (for each species) x 6 (all-against-all among four wells = 6) = 12 pairwise comparisons for which the adjustatment of multiplicity has been undertaken.
Note that \code{linfct = c(Pairs.Well = 1)} would yield the same result as \code{linfct = c(Pairs = 1)} shown in the example above.

When dealing with more complex models also keep in mind that the numeric vector in \code{linfct} can refer to the position of any variable, or set of variables obtained by joining, within \code{model}.


\subsubsection[User-defined comparisons of interest]{User-defined comparisons of interest}\label{use-def-comp}

For performing even more specific comparisons of interest, the user has the possibility to provide a contrast matrix directly. A contrast is defined as a linear combination of two or more factor level means (averages) whose coefficients add up to zero \citep{hochberg}. 
...
<<label=mcp-summary-linfct>>=
summary(vaas.G06.mcp)$linfct
@
...
<<label=contrast-matrix-I, results=hide>>=
...
An according call of the \code{opm\_mcp()} function after selecting some comparisons of interest would then be:

<<label=mcp-wells, results=hide>>=
...
\begin{figure}
\scalebox{0.9}{
<<label=vaas4-mcp-figure, fig=TRUE, echo=FALSE, width=9, height=3>>=
...
<<label=mcp-output-data, results=hide>>=
...
\begin{figure}
\scalebox{0.9}{
<<label=vaas-G06-mcp-figure, fig=TRUE, echo=FALSE, width=8, height=4>>=
...
<<label=mcp-G06-output-contrast, results=hide>>=
...
<<label=mcp-G06-user-def-contrasts, results=hide>>=
...
\begin{figure}
\scalebox{0.9}{
<<label=mcp-G06-user-def-contrasts-figure, fig=TRUE, echo=FALSE, results=hide, width=8, height=3>>=
...
\subsection[Discretization]{Discretization}\label{application discrete}

After calculating curve parameters and optionally generating a suitable subset, data can be discretized and optionally also exported for analysis with external phylogeny software or for inclusion into a scientific manuscript as text or table.
In the \pkg{opm} manual and help pages, the functions relevant for either task are contained in the families ``discretization-functions'', ``phylogeny-functions'' and partially also in ``naming-functions'', with according cross-references.
...
\subsubsection[Discretization and phylogenetic data export]{Discretization and phylogenetic data export}\label{application discrete-phylogeny}
...
<<label=list-discretize, results=hide>>=
...
<<label=phylo-data-text, results=hide>>=
...
<<label=phylo-data-html, results=hide>>=
...
The default settings of \code{do\_disc()} imply exact k-means partitioning into three groups (``negative'', ``ambiguous'' and ``positive''), treating all contained plates together, and using the maximum-height parameter for discretization.
...
In this sense, the results will be consistent, but there are not many other things the algorithm guarantees.
...
This is usually unproblematic if the data contain both really negative and really positive reactions, but data that in reality are negative throughout, or uniformly positive, would nevertheless be split into three (or two) clusters.
That is, additionally checking the curve heights and particularly the ``cutoffs'' entry obtained \textit{via} \code{disc\_settings()} should initially be mandatory.

It is also possible to make the reactions uniform within metadata-defined groups.
This would be specified with the \code{unify} argument and would deliberately deviate from the kind of consistency described above.
The unification approach replaces the primary discretization results with the most frequent value within the respective combination of group and well if this value is present in a given proportion of the original values and with \code{NA} otherwise.
The according cutoff is set using \code{opm\_opt(min.mode = ...)} or directly.
Thus there are two distinct meanings of ``ambiguous'' reactions, as ambiguity either results from the clustering of the parameters, or by clustering results that deviate between distinct experimental replications.
It is unnecessary and perhaps not preferable to use both approaches together, i.e. to cluster into three groups only and then also unify.
Note that \code{listing()} and \code{phylo\_data()} would use the same unification approach, if requested.
...
<<label=opm-css-file, results=hide>>=
...
<<label=opm-html-file, results=hide>>=
...
<<label=wd-css-file, results=hide>>=
...
<<label=prep-disc-data,results=hide>>=
vaas.repl <- subset(vaas_et_al,
  query = list(Experiment = c("First replicate", "Second replicate")))
...
The here presented \pkg{opm} package for the free statistical software \proglang{R} \citep{r} features not only the calculation of aggregated values (curve parameters) including their (bootstrapped) confidence intervals, but also provides a rather complete infrastructure for the management of raw kinetic values and curve parameters together with any kind of meta-information of relevance for the user \citep{vaas, Vaas2013}.
...
The package can also generate \proglang{CSV} output files, but due to the limitation of this format these files cannot be read back into \pkg{opm} in a meaningful way (but into \proglang{R}).
...
At the moment only the translation of well coordinates to substrate names is provided, as well as access to MeSH names and CAS, KEGG and Metacyc IDs (and the according web pages) for the majority of the substrates.
...
To summarize, we are convinced that the \pkg{opm} package already enables the users to analyse OmniLog\textregistered \ PM data in rather unlimited exploratory directions \citep{vaas, Vaas2013}.
