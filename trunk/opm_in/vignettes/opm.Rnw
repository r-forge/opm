...
Shown are the results from one plate on the three wells A01 (Negative Control), A02 (Dextrin) and A03 (D-Maltose) as indicated by the sub-plot titles.
}
\end{figure}
...
\code{norm.per = c("none")} applies no normalisation and returns a dataframe with \code{as.groups = TRUE} containing means computed of estimators according the available metadata (see Fig~\ref{fig:ci_plot_groups}). 
...
Normalisation subtracting plate means (\code{norm.per = c("row")}) or well means (\code{norm.per = c("column")}) can be applied and the results directly plotted using \code{ci\_plot()}.
...
Moreover, \textit{via} \code{norm.by} it is possible to use selected well(s)/plate(s) for calculation of means used for normalisatio. 
...
\textit{Via} the argument \code{mcp.def} the user can define the type of contrast matrix to be used for the multiple comparison. In principle, the contrast matrix defines which group means should be compared and how. In our example Tukey-type contrast-matrix indicates an all-against-all comparison, resulting in a set of six two sided pairwise comparisons amoung all the four strain-means (all possible pairs).
...
\rcscom{
Is it possible to show here at least part of the output from \code{summary(xx)}?
...
For example, the last comparison showing the difference of mean of curve parameter A values of strain DSM 30083T minus the mean of A values of strain DSM18039 results 26.615 units. Therefore, the point estimator of this difference of specific group-means is plotted at position 26.615 on the x-axis. That is, in mean A-values for Strain DSM30083T is 26.615 units larger than the mean A-value of strain DSM18039 (for the detailed numeric outcome see the estimate results provided by \code{summary(xx)} or \code{confint(xx)}).
...
In pairwise comparisons where the 95\% confidence interval crosses the vertical dashed line at zero there is no significant difference between the group means and respectively, if zero is not included, a significant difference is shown. Then, the more distant the 95\% confidence interval is from zero (dashed vertical line in the plot), the larger the biological effect size, i.e. the real differences between the groups. It has to be pointed out, that effect sizes are not correlated with sizes of the corresponding p-values: In this example the results from the statistical calculations indicate that all significant differences are highly significant (p < 0.001). 
...
<<fig=FALSE, width=8>>=
op <- par(no.readonly = TRUE) # save default plotting settings
...
95\% confidence interval for comparison of group means between the four strains. All pairwise comparisons are shown. The filled black circle indicates the point estimator of difference betweeen the mean of groups. 
}
\end{figure}
