...
Visualization of the results is available by the \code{plot()}-method for \code{glht}-objects:
\begin{figure}
\scalebox{0.9}{
...
It has to be pointed out, that the \code{model}-statement defines, which group-means are available for comparisons. In the following example \code{Species} contains only two levels (``Pseudomonas aeruginosa'' and ``Escherichia coli''). Thus, irrespectively the stated contrast-type, only one comparison is possible.
