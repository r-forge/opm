...
All implementations also provide confidence limits calculated \textit{via} bootstrapping, with 95\% being the default confidence value \citep{efron}.
...
Users come in direct contact only with \proglang{OPM}, \proglang{OPMA}, \proglang{OPMD} and \proglang{OPMS} classes.
A number of \proglang{S3} helper classes is also used by several functions.
...
The non-virtual classes in the right part of the figure are either well-known in \proglang{R} (matrices and arrays) or not directly manipulated by the user (\proglang{CMAT}).
}
\end{figure}
...
%% END
\rcsadd{Alternatively, curve parameters can be extracted using a spline-based
  fitting procedure implemented in \pkg{opm}. Three different modeling
  alternatives for the splines exist: (Low rank) cubic smoothing splines
  \citep{Reinsch:1967} as implemented in \code{smooth.spline}, thin plate
  splines \citep[a generalization of smoothing splines]{Wood2003} and P-splines
  \citep{eilers}; the latter two are implemented in the package \pkg{mgcv}.
  % + package citations
}
...
%TODO add own picture explaining these parameters?
...
%TODO update this figure; maybe even merge it with figure 1
...
%TODO a problem with the subsections is that they are not numbered accordingly
%
...
%Since a PM experiment usually contains sets of plates, which might not be measured all in the same experimental run, it is important to consider possible systematic variations and control for those.
...
\subsection[Statistical comparisons of group means]{Statistical comparisons of group means}


%% BEGINJohannes
{\color{red}
Besides statistically comparing single curves using \code{ci_plot()}, the user may also be interested in statistically comparing the mean value of distinct groups of curves. 
For example, imagine the comparison of four different bacteria using GEN-III microplates.
For each bacterial strain, ten replicates have ben performed {\color{red}(appropriate example datasets are distributed with the \pkg{opm} and \pkg{opmdata} packages).} 
Do these four bacteria differ in, e.g., the mean value of, e.g., curve parameter A of, e.g, well A1?
Here, a statistical comparison of four groups (four organisms) of each 10 values (curve parameter A of 10 replicates of well A1) will be perfomed. Statistically, this results in simultaneous inferences across multiple questions \citep{hothorn}.
}
...
By referring to available metadata and/or the substrate names, the user is able to define groups of interest (e.g., four different bacteria), set up a model of choice and perform multiple comparison of group means on individually specified contrasts \citep{bretz, hsu}. {\color{red} The choice of appropriate models and contrasts will be explained in detail below.}
...
% describe here, why multiple comparisons of means, why not ANOVA, why multcomp and nothing else?
At this point, {\color{red} it is necessary} to highlight the power and flexibility of simultaneous multiple comparison procedures and encourage the user to apply contrast tests on individually designed sets of mean comparison rather than to employ the probably {\color{red} more popular} classical ANOVA-based approaches followed by F-tests.
It has to be pointed out that in general F-tests provide no more than {\color{red} only} global information about main effects and interaction effects.
...
But if an increasing number of hypotheses is tested{\color{red}, i.e., an increasing number of groups is compared to each other}, with the number of true hypotheses unknown, the probability of at least one wrong testing decision also increases {\color{red} (i.e., the claim, that any two out of several groups may differ significantly, can be wrong)}.
...
%TODO explain the implementation of how factors are merged to produce a pseudo-one-way-layout \citep{schaarschmidt}.
...
\code{vaas\_1} already contains aggregated data but we will \rcsdel{here} re-calculate some for demonstration purposes.
...
This will only estimate two of the four parameters \rcsmark{add which}.
...
\rcsadd{One can also specify different spline fitting methods using \code{method = "spline"}.
...
@
\rcsadd{Other spline types can be specified \textit{via} the \code{type} argument in the function \code{set\_spline\_options}.}
...
{\color{blue} In order to demonstrate the functionality of \code{opm\_mcp()}, let  us make use of the example addressed above. 
Imagine the comparison of four different bacteria using GEN-III microplates.
For each bacterial strain, ten replicates have ben performed.
Do these four bacteria differ in, e.g., the mean value of, e.g., curve parameter A of, e.g, well A1?
Here, a statistical comparison of four groups (four organisms) of each 10 values (curve parameter A of 10 replicates of well A1) will be perfomed.
Let us first subset from the data set \code{vaas\_et\_al} the ten GEN-III plates from the first technical replicates and let us also subet for the curves from well G06.
<<results=hide, width=8>>=
xyz <- subset(vaas_et_al[,,"G06"], list(Experiment = "First replicate"))
@

Let us plot these data (see Figure~\ref{fig:xy_plot_mcp}). For each of the four strains, ten curves are shown. 

<<fig=FALSE, width=8>>=
xy_plot(xyz, include = "Strain", neg.ctrl = FALSE, legend.fmt  = list(space = "right"))
@
...
xy_plot(xyz, include = "Strain", neg.ctrl = FALSE, 
...
Is there between any of the four strains a significant difference, based on the mean of the curve parameter A values as determined for each of the ten curves?
Here, a multiple comparison of group means is performed using \code{opm\_mcp()}.
The groups to be compared are defined by the argument model. (((Die anderen Argumente sollten hier auch erklaert werden, oder?)))
<<results=hide, width=8>>=
(x <- opm_mcp(xyz, model = list("Strain"), m.type = "aov",
...
For example, the mean of curve parameter A values of strain DSM 30083T is 26.615 units larger than the mean of A values of strain DSM18039 (see the estimate results above). 
...
However, the position of the 95\% confidence interval indicates that the difference between strains DSM18039 and 429SC1 is much larger than between strains DSM30083T and 429SC1.
