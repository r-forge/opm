...
\rcsadd{To support a fast and easy data exchange of the resulting complex data bundles its conversion into the \pkg{opm} \proglang{YAML} format is facilitated. Section~\ref{methods batch} describes the procedure and example code from Section~\ref{application batch} facilitates the method's direct application.}
...
See Section~\ref{application settings} for details.
...
To this end, the \pkg{opm}-user can integrate the metadata into \proglang{OPM} and \proglang{OPMS} objects using the function \code{include\_metadata()} (among other functions for this task; see Figure~\ref{fig:work_flow}).
...
Optionally, normalization by either the subtraction of or division by the plate-wise or well-wise means (i.e., either per row or per column) can be conducted beforehand.
...
In general, F-tests \textit{only} provide global information about main effects and interaction effects.
...
\rcscom{
Lea, please explain the implementation of how factors are merged to produce a pseudo-one-way-layout \citep{schaarschmidt}, and/or remove this note.
}
...
This script is guaranteed to run at least under \proglang{UNIX}-like operating systems; see the documentation of \proglang{Rscript} for details.
...
files
@
Then check whether this returned a vector of nine file names.
...
The file containing multiple OmniLog\textregistered \ plates would need to be split beforehand using \code{split\_files}.
...
The function \code{heat\_map()} must be applied to matrices or data frames constructed using the helper-function \code{extract()}:
...
Additionally, using the function \code{ci\_plot()} the user is enabled to visualize parameters'point estimators and corresponding 95\% confidence intervals, derived \textit{via} bootstrapping during aggregation of raw kinetic data into curve parameters. 
Thereby the bracket operator as described above (see Section~\ref{application manipulation}) facilitates the free selection of subsets of interest. 
In an example Figure~\ref{fig:ci_plot} provides such a visualization of the parameter maximum height (A) for the three wells A01 (Negative Control), A02 (Dextrin) and A03 (D-Maltose) from one plate (the sixth plate of the first biological repetion from dataset \code{vaas_et_al}).
...
\scalebox{1.225}{
<<fig=TRUE, echo=FALSE, results=hide, width=9, height=6>>=
print(ci_plot(vaas.1.6[, , c("A01", "A02", "A03")], as.labels = list("Species", "Strain"), subset = "A", legend.field = NULL, x = 170, y = 3))
@
}
\caption{
\label{fig:ci_plot}
...
Furthermore, the helper-function \code{extract()} offers flexibility to apply different approaches of normalisation and grouping of comprehensive OPMS-objects according selected metadata, as introduced in Section~\ref{methods plotting-aggregated}.
As introduced in Section~\ref{methods plotting-aggregated} the purpose of normalization in an PM-experiment is to minimize systematic variations in the aggregated curve parameters so as to more easily recognize biological differences, as well as to allow for the comparison of parameters across plates processed in different experimental runs.
The underlying ideas are mainly derived from DNA-microarray experiments for measuring gene-expression levels \citep{quackenbush}.

After the extraction of the values together with demanded metatdata (argument \code{as.labels}), the resulting dataframe can be treated by \code{extract()} for generating dataframes with joined metadata for grouping (indicated by \code{as.groups}) and applied normalisation \textit{via} the argument \code{norm.per}. 

<<results=hide, fig=FALSE, width=10>>=
x <- extract(vaas_et_al, as.labels = list("Species", "Strain"),
  dataframe = TRUE)
@
For a better understanding of the following applications of \code{extract()} it is highly recommended to take a look on the structure of the resulting dataframe.

\code{norm.per = c("none")} applies no normalisation and returns a dataframe with \code{as.groups = TRUE} containing means computed of estimators according the available metadata (see Fig~\ref{fig:ci_plot_groups}). (Note: the bracket operator \code{[]} addresses the respective columns in the data frame \code{y} produced using \code{extract()}).
...
print(ci_plot(y[,c(1:7, 13)], legend.field = NULL, x = 350, y = 0))
@
}
\caption{
\label{fig:ci_plot_groups}
Comparison of mean point estimates and their 95\% confidence intervals, computed over "Species" and "Strain", for the parameter maximum height (A) observed from four strains, using \code{ci\_plot()}.
Shown are the results on the three wells A01 (Negative Control), A02 (Dextrin), A03 (D-Maltose), A04 (D-Trehalose) and A10 (Positive Control) as indicated by the sub-plot titles. Point estimates that have no overlapping confidence intervals are regarded to be significantly different.
}
\end{figure}

Normalisation subtracting plate means (\code{norm.per = c("row")}; by default, this would subtract the mean of each plate from each of its values (over all wells of that plate)) or well means (\code{norm.per = c("column")}; by default, this would subtract the mean of each well from each of its values (over all plates in which this well is present)) can be applied and the results directly plotted using \code{ci\_plot()}.

<<results=hide, fig=FALSE, width=10>>=
# without normalisation (no figure is plotted in this manual)
y <- extract(x, as.groups = TRUE,  norm.per = c("none"))
ci_plot(y[,c(1:7, 13)], legend.field = NULL, x = 350, y = 1)
# normalisation by plate-means (no figure is plotted in this manual)
y <- extract(x, as.groups = TRUE,  norm.per = c("row"))
ci_plot(y[,c(1:7, 13)], legend.field = NULL, x = 130, y = 1)
# normalisation by well-means (no figure is plotted in this manual)
y <- extract(x, as.groups = TRUE,  norm.per = c("column"))
ci_plot(y[,c(1:7, 13)], legend.field = NULL, x = 20, y = 1)
@

Moreover, \textit{via} \code{norm.by} it is possible to use selected well(s)/plate(s) for calculation of means used for normalisation. 
With \code{direct = TRUE} even directly entered numeric values can be used for normalisation purposes. 
See Figure~\ref{fig:ci_plot_norm} for an example of plotted CIs normalized by subtracting the value of well A10 (Positive Control). 
Note that due to the internal structure of the dataframe \code{norm.per = "row"} in combination with the \code{norm.by}-argument has to be used.


<<results=hide, fig=FALSE, width=10>>=
# normalisation by subtraction of well-mean of A10
y <- extract(x, as.groups = TRUE,  norm.per = c("row"), norm.by = 10,
  subtract = TRUE)
...
Comparison of mean point estimates and their 95\% confidence intervals, computed over ``Species'' and ``Strain'' and normalized by subtraction of the overall well-mean of A10 (Positive Control) for the parameter maximum height (A) observed from four strains, using \code{ci\_plot()}.
...
To demonstrate the functionality of \code{opm\_mcp()}, a comparison of four different bacteria, with each ten replicates using GEN-III microplates, will serve as an example. 
One experimental questions, which could be addressed might be ``Do these four bacteria differ in, e.g., the mean value of, e.g., curve parameter A of, e.g, well G06''?
The answer will be given by perfoming a statistical comparison of the four groups (four organisms) against each other. 
...
Now the dataset contains four strains measured each in ten replicates, represented by ten curves per strain (see Figure~\ref{fig:xy_plot_mcp}). 
...
The statistical question to be addressed is: ``Is there a significant difference between any of the four strains, comparing the mean of their curve parameter A values (determined for each of the ten curves)?''

Here, a multiple comparison of group means is performed using \code{opm\_mcp()}.
The groups to be compared are defined by the argument \code{model}.
The argument \code{m.type} specifies the model type to be used for fitting the mode, e.g., a linear model (\code{lm}), a generalized linar model (\code{glm}), or an analysis of variance model (\code{aov}).
...
In our example Tukey-type contrast-matrix indicates an all-against-all comparison, resulting in a set of six two sided pairwise comparisons amoung all the four strain-means (all possible pairs).
A more detailed explanation of contrast matrices is shown further below.


<<results=hide, width=8>>=
(xx <- opm_mcp(vaas.subset.mcp, model = ~ Strain, m.type = "aov",
...
\rcscom{
Is it possible to show here at least part of the output from \code{summary(x)}?
Markus, can we adapt the summary function, that only the model-fit is shown without the data?
}

\code{opm_mcp()} internally calls \code{glht} from package \pkg{multcomp} and returns an object of class \code{glht}. The results of the performed statistical test can be plotted (shown in Figure~\ref{fig:ci_plot_mcp}) using the methods available for objects of class \code{glht} (see help-page of \code{glht} for details).
On the y-axes the performed comparisons are indicated as differences of the groups, determining which differences of means are computed.

For example, the last comparison showing the difference of mean of curve parameter A values of strain DSM 30083T minus the mean of A values of strain DSM18039 results 26.615 units. 
Therefore, the point estimator of this difference of specific group-means is plotted at position 26.615 on the x-axis. 
That is, in mean A-values for Strain DSM30083T is 26.615 units larger than the mean A-value of strain DSM18039 (for the detailed numeric outcome see the estimate results provided by \code{summary(xx)} or \code{confint(xx)}).
Additionally, each point estimator for difference of means comes with an 95\% confidence interval providing information about statistical significance of the test, the effect size and the variability of the mean-differences.

In pairwise comparisons where the 95\% confidence interval includes zero there is no significant difference between the group means and respectively, if zero is not included, a significant difference is shown. 
Then, the more distant the 95\% confidence interval is from zero (dashed vertical line in the plot), the larger the biological effect size, i.e. the real differences between the groups. 
It has to be pointed out, that effect sizes are not correlated with sizes of the corresponding p-values: In this example the results from the statistical calculations indicate that all significant differences are highly significant (p < 0.001).
However, the position of the 95\% confidence interval indicates that the difference between strains DSM18039 and 429SC1 (i.e, the effect size in the second comparison) is much larger than between strains DSM30083T and 429SC1 (smaller effect size in the third comparison).
...
\rcscom{Lea, explain contrasts, refer to \code{contrMat() from \pkg{multcomp}}}
...
\rcscom{comment Johannes: Lea, could it make sense to explain the contrast matrix using the following example? I still not have understood for what these 0, 1, and -1 are.}

The principle of a contrast matrix can best be explained using the example from above (see Figure~\ref{fig:ci_plot_mcp}). If I had any idea on that stuff, I would have given an appropriate explanation for the code output seen below.
<<width=8>>=
summary(xx)[2]
@


\rcscom{Lea, comparison according metadata 'Species'}
<<results=hide, width=8>>=
x <- opm_mcp(vaas_4, model = ~ Species, m.type = "lm",
  mcp.def = mcp(Species = "Dunnett"))
@

\rcscom{Lea, explain confidence intervals for differences of group means, point out the difference to the 95\%CI of the curve-parameter estimators.
Figure:
...
\rcscom{Lea, explain: more sophisticated with manually defined set of contrast:}
