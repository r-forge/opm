...
% Avoid repetition (of the forthcoming chapters) as far as possible.
...
To facilitate a comprehensive and straightforward data processing and analysis, the raw and aggregated data of each single kinetic can be concatenated and combined with metadata (see Figure~\ref{fig:data}).
...
Four descriptive curve parameters are estimated, which is explained in detail in Section~\ref{methods aggregating}, whereas example code for curve parameter aggregation is given in Section~\ref{application aggregating}.
...
\rcsadd{For visualization purposes the \pkg{opm} package comes with functionalities for plotting both the raw and the aggregated data. The raw kinetic data can be plotted either as level plots or as XY plots, as explained in Section~\ref{Plotting functions for raw data}.
The estimated curve parameters can be plotted either as confidence-interval plots, radial plots  or heat maps, which is shown in Section~\ref{Plotting the aggregated data}.
See Section~\ref{application raw-plotting} and ~\ref{application plotting-aggregated}, respectively, for plotting example code.}

To statistically compare curve parameters, tools for the multiple comparison of groups means have been adapted to PM data.
This allows for testing statistical hypothesis involving groups of numerous plates \rcsadd{or wells} as defined, e.g., \textit{via} \rcsadd{the substrate information or} the stored metadata entries.
...
\rcsadd{Further,} the aggregated data can be discretized and exported for phylogenetic analysis or reconstruction of character evolution with external phylogeny software.
...
Also, a typical application of the OmniLog\textregistered \ technology is to classify reactions as either ``positive'', ``negative'' or ``weak'' (ambiguous). 
The discretization methods implemented in \pkg{opm} are described in Section~\ref{methods discrete-binary}.
Example code for these discretization applications, including the export of discretization results as  publication-ready tables, is included in Section~\ref{application discrete-table}.
...
It has to be emphasized that metadata can include all kind of describing characteristics of the observed organism(s) such as taxonomic affiliation, geographical and/or ecological origin, or of the performed experimental setting such as culture conditions, genetic modifications, physiological information of any kind and so on.
...
For an unambiguous match between the raw kinetic data in the \proglang{OPMS} object and the collected metadata, a unique identifier is needed: This is, by default, proviced by the combination of \textit{Setup Time} and \textit{Position}, which should unequivocally identify certain plates.
...
To facilitate the user-friendly compilation of metadata information, \code{collect\_template()} generates a data frame (and additionally, if requested, a CSV file) in which each line represents a single PM plate.
The function \code{collect\_template()} automatically includes the \textit{Setup Time} and \textit{Position} (or any other CSV data of interest) of each plate into the data frame or file providing a structured template for input of metadata.
Thus, it is easy to subsequently add further columns describing any kind of meta-information of interest of any PM plate of interest.
The resulting data frame or CSV file can then be queried for the information specific to each plate, and the resulting data integrated into \proglang{OPM} or \proglang{OPMS} objects using \code{include\_metadata()}.
...
\subsection[Plotting functions for raw data]{Plotting functions for raw data}\label{Plotting functions for raw data}
...
\subsection[Plotting the aggregated data]{Plotting the aggregated data}\label{Plotting the aggregated data}
...
\subsection[Plotting functions for raw data]{Plotting functions for raw data}\label{application raw-plotting}
