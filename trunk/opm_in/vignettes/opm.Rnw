...
But the most recent versions of the OmniLog\textregistered \ software (contact your local representative of the vendor for this) can batch-export one plate per CSV file, and \pkg{opm} can split multi-plate CSV into files that can be input by the package.
...
Because the definition of groups using stored metadata is highly flexible, the user is enabled to individually define sets of mean comparisons, i.e., to set up user-defined contrast tests \citep{hsu}.
...
% Please keep the following paragraphs a brief as possible, but include cross-references.
...
This includes storage of the \pkg{opm} data in the \proglang{YAML} format, as detailed in Section~\ref{methods batch}.
...
This allows the user to adopt \pkg{opm} functionalities to personal preferences and to thereby substantially ease coding effort.
...
Users come in direct contact only with the \proglang{OPM}, \proglang{OPMA}, \proglang{OPMD} and \proglang{OPMS} classes (see Figure~\ref{fig:class_hierarchy} and Figure~\ref{fig:overview}).
...
The few metadata that come along with the imported CSF file can be accessed via \code{csv_data()}.
However, for most experimental designs it is clearly necessary to add much more meta-information to the kinetic data.
...
% mgcv seems to already be cited, as Wood2003 is listed among the package citations on CRAN
...
This has successfully been applied to slow-growing organisms in the bacterial genus \textit{Geodermatophilus}, which had to be measured three times consecutively in the OmniLog\textregistered \ instrument, in total, up to 12 days \citep{montero, montero2013}.
...
Beginning with one to several CSV files containing raw kinetic data exported by the proprietary OmniLog\textregistered \ software \textit{File Management/Kinetic Analysis}, or YAML files that have been generated in previous \pkg{opm} runs, a variety of work flows are possible for setting up  \proglang{OPM} or \proglang{OPMS} objects.
...
For the graphical representation of the aggregated data, namely point estimators and corresponding confidence limits for the curve parameters of individual curves, the function \code{ci\_plot()} provides a framework to plot subsets of different parameters in a convenient and easily applicable manner.
...
When analysing empirically obtained measurements such as PM data it is important to consider possible systematic variations and to control for those by normalizataion.
...
Optionally, normalization by either the subtraction of or division by the plate-wise (accross all 96 wells) or well-wise means (accross all plates that contain this well) can be conducted beforehand.
...
To evaluate the differences of treatment means where the groups are defined by more than one metadata entry and thus may result in quite complex models, we suggest to set up \textit{cell-means-models} (also known as \textit{pseudo-one-way-layouts} as discussed in \citep{schaarschmidt}). 
In this approach estimators for treatment and variance are derived from a model with all treatments (here: metadata) combined in a single factor. 
Technically this combination of factors can be done using \code{interaction()} or \code{paste()}.

\rcscom{
But this is never explained, and it is left unclear at which stage and to which kind of object either function could be applied.
}
...
Discretization is regarded to be the process of transferring continuous data into discrete counterparts. For example, continuous values ranging from 0 to 400 could arbitrarily be discretized into the three states ``low'' (from 0 to 100), ``intermediate'' (from 101 to 200), and ``high'' (from 201 to 400).
For instance, discretizing the data is necessary for analysing them with external programs that cannot deal with continuous characters.
...
A usage example is provided in Section~\ref{application discrete-table}.
...
files
@
Then check whether this returned a vector of nine file names, which includes also the path where they are located on the local hard disc.
(It might fail in very unusual \proglang{R} installation situations; in that case, the files must be found manually.)
For demonstration purposes, the testdata contain data from EcoPlate\textregistered \, Gen-III, PM01 and PM20 plate types. 
...
For instance, for inputting all example files (including different plate types, but except for the one with multiple plates), consider the following code:
...
Details on all three processes are given in the according sections, and for the exact use of these arguments see the \pkg{opm} manual, entering \code{?batch\_opm\_to\_yaml}.
...
As an example, we here add an (arbitrary) \textit{Colour} column with the values ``blue'', ``red'' and ``yellow'' and another (arbitrary) \textit{Integers} column with the integer values ``10'', ``20'' and ``30'':
<<width=10>>=
metadata.example[, "Colour"] <- c("blue", "red", "yellow")
metadata.example[, "Integers"] <- c(10, 20, 30)
@
...
One might want to remove the file is is not needed any more:
<<width=10>>=
...
(One of the fields in the interface of the \textit{Data Collection} software of the OmniLog\textregistered \ reader was named ``Sample Number'', but the user entered species and strain designations, which obviously not really fits to ``Sample Number'').
...
As result, a new metadata entry is created, with ``Organism'' as key, and which contains the same values as the metadata entry having the key ``Sample Number''.
...
To achieve this, the following technique was implemented.
Here, it is useful that all operators (except for \code{\$} and other high-precedence operators, which can be used for defining nested keys) on the left side, if present, would be changed into a call to \code{list()}.
...
Thus we could clean up our metadata in a single line of code:
...
Assigning \code{NULL} to a metadata key would remove that entry.
...
So keep in mind that formulas are very flexible for modifying metadata entries.
...
The following code adds (1) a new metadata entry with the key ``Factor'' and into which the integer values of the metadata entry with the key ``Integers'' are coerced to factors, (2) a new metadata entry with the key ``Character'' and into which the integer values of the metadata entry with the key ``Integers'' are coerced to characters, and (3) a new metadata entry with the key ``Integers10'' and into which the integer values of the metadata entry with the key ``Integers'' are transferred as integers but multiplied by the value of 10. See the \code{?map\_metadata} for details.
<<results=hide>>=
# include metadata from the previously compiled data frame
example.opm <- include_metadata(example.opm, md = metadata.example)
metadata(example.opm)
# (1) 
example.opm <- map_metadata(example.opm, Factor ~ as.factor(Integers))
# (2)
example.opm <- map_metadata(example.opm, Character ~ as.character(Integers))
# (3)
example.opm <- map_metadata(example.opm, Intergers10 ~ (Integers * 10))
metadata(example.opm)
@
...
As this is a time-consuming intensive process (particularly if \pkg{grofit} is used), it may be split over several cores on a multicore machine if the \pkg{multicore} \proglang{R} package is available by setting the cores argument to a value larger than one.
...
At moment, several colour palattes are implemented for curve colour (which can be accessed with \code{?select_colors}) but which have a maximum of 24 different colour. In case the user needs more colours, an own colour vector needs to be designed by the user, using the argument \code{col} within the function \code{xy_plot()}.
...
print(heat_map(vaas.1.6, as.labels = "Strain", as.groups = "Species"))
@
}
\caption{
\label{fig:heat_map}
...
The default colour setting uses topological colours, with deep violet and blue indicating the lowest values and light brown indicating the highest values, but the colour palette also can be individually designed by the user.
}
\end{figure}
...
<<results=hide, fig=FALSE, width=10>>=
# without normalisation (figure not shown)
y <- extract(x, as.groups = TRUE,  norm.per = "none")
ci_plot(y[, c(1:7, 13)], legend.field = NULL, x = 350, y = 1)

# normalisation by plate means (figure not shown)
y <- extract(x, as.groups = TRUE,  norm.per = "row")
ci_plot(y[, c(1:7, 13)], legend.field = NULL, x = 130, y = 1)

# normalisation by well means (figure not shown)
y <- extract(x, as.groups = TRUE,  norm.per = "column")
ci_plot(y[, c(1:7, 13)], legend.field = NULL, x = 20, y = 1)
@
...
\scalebox{1.225}{
<<fig=TRUE, echo=FALSE, results=hide, width=9, height=8>>=
print(ci_plot(y[, c(1:7, 13)], legend.field = NULL, x = 0, y = 0))
@
}
\caption{
...
%\rcscom{
%Vorschlag Johannes: Wir sollten hier ein plakatives Beispiel nehmen, in dem wir sowohl Gruppenvergleiche haben, die nicht verschieden voneinander sind, wie auch Gruppenvergleiche, die signifikant verschieden voneinander sind und bei denen man auch einen Unterschied in der effect size sehen kann. Daher sollten wir auf den \code{vaas\_et\_al} Datensatz zurueckgreifen, da dort eine Fuelle an Kurven zur Verfuegung steht.
%Die beiden Beispiel-Figures von Lea \ref{fig:opm_mcp_CI_plotI}, \ref{fig:opm_mcp_CI_plotII}) halte ich nicht fuer so ideal, denn \ref{fig:opm_mcp_CI_plotI} beinhaltet nur einen paarweisen Vergleich, kein multiple comparison, und in \ref{fig:opm_mcp_CI_plotII} gibt es keinen Unterschied bei irgendeinem der paarweisen Vergleich - es wird dem Leser das Potential einer solchen Abbildung nicht vollstaendig klar.
%}

To demonstrate the use of \code{opm\_mcp()}, the function for conducting comparisons of group means directly on \proglang{OPMS} objects, we conduct a comparison of four distinct strains, each of which represented by ten replicates of GEN-III microplate measurements.
...
\textit{Via} the argument \code{mcp.def} the user can define the type of contrast matrix to be used for the multiple comparison. 
...
\code{opm_mcp()} uses \code{glht()} from the package \pkg{multcomp} and returns an object of class \code{glht}.
...
par(mar = c(3, 15, 3, 2)) # adapt margins in the plot
plot(vaas.subset.mcp)
par(op) # reset to default plotting settings
@
...
95\% confidence intervals for are indicated by horizontal bars and parentheses. Note the difference in interpretation of this figure compared to the \code{ci\_plot()} Figures in Section~\ref{application plotting-aggregated} (see also the main text).
}
\end{figure}
...
The interpretation of this output is that there is no significant difference in the mean of the curve parameter A values of DSM30083T and DSM1707, whereas in all other pairwise comparisons the difference between group means is highly significant (p < 0.001).
...
In the here shown example all group means are significantly different from each other, except for the comparison of strains DSM 30083\textsuperscript{T} and DSM 1707 (Figure~\ref{fig:ci_plot_mcp}).
...
For a meaningful biological interpretation of the results it is therefore highly recommended to take into account also the effect size and not only the p-values.
...
To demonstrate the principle of a contrast matrix, an all-to-all comparison (a ``Tukey''-type contrast) of four groups is assumed. 
<<width=8>>=
n <- c(10,20,30,40)
 names(n) <- paste("group", 1:4, sep="")
 contrMat(n, type = "Tukey")
@
The output table reads as follows. In the first line, the ``1'' and ``-1'' indicate that from the means of group2 the means of group1 are substracted. Thus, for each line, a pair of groupwise comparisons is defined. The function \code{contrMat()} from \pkg{multcomp} (see \code{?multcomp::contrMat}) provides an overview of the predefined contrast types that can be used in the \code{opm\_mcp()} argument \code{mcp.def}.
...
Although the user typically expects the order of these wells in the OPMS object to be A01 -> A02 -> A03 -> A04, is is essential to control the order of the elements before setting up manually a contrast matrix:
<<width=8>>=
wells(vaas_4[, , 1:4], full = TRUE)
@

The model will therefore contain \code{Well} as factor-variable and four possible comparisons of interest can be defined like:
...
The resulting call of the \code{opm_mcp()} function would then be:
<<results=hide, width=8>>=
x <- opm_mcp(vaas_4[, , 1:4], model = ~ Well, m.type = "lm", mcp.def = contr)
@


The correct setup of the contrast matrix can be controlled by typing
<<results=hide, width=8>>=
xy <- summary(x)
xy$linfct
@
...
\begin{figure}
\scalebox{0.9}{
<<fig=TRUE, echo=FALSE, results=hide, width=9, height=5>>=
...
In the following example \code{Species} contains only two levels (``\textit{Pseudomonas aeruginosa}'' and ``\textit{Escherichia coli}''). 
...
\rcscom{
The options for more easily defining mcp.def are not explored in the chapter!
}
...
This object can be returned (and the multiple comparisons omitted) by using \code{do.mcp = FALSE}:
...
The \code{subset()} function has a \code{positive} argument that allows one to create a subset containing only the wells that were positive in at least one plate or in all plates, as well as a \code{negative} argument (see the argument \code{invert = TRUE}).
See the manual for details, using \code{help(subset, package = "opm")}.
...
The results from discretization into positive, negative and ambiguous could directly be used for subsetting. For example, the following code plots only those wells in which all curves have been classified by k-means partitioning to yield a positive reaction:
<<results=hide, width=8>>=
xy_plot((subset(vaas_4, positive = "all")), neg.ctrl = NULL)
@
...
\proglang{CSS} (Cascading Style Sheets) is a style sheet language used for describing the look and formatting of a document written in a markup language, e.g., in \proglang{HTML}.
...
The legend of the table is as used in taxonomic journals such as \textit{International Journal of Systematic and Evolutionary Microbiology} (IJSEM, http://ijs.sgmjournals.org/) but could also be adapted.
