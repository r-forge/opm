...
\subsubsection[General principle]{General principle}\label{statcomp general}
...
The following section will demonstrate how to apply the powerful \code{opm\_mcp()} functionality. As explained in section \ref{methods group-means}, this function internally reshape the data into a ``flat'' data frame containing one column for the chosen parameter value, one column for the well (substrate) name and optionally additional columns for the selected metadata.
For performing the testing procedure, a model has to be stated that specifies the factor levels that determine the grouping \citep{searle, hothorn}.

The \code{opm\_mcp()} function allows for applying such multiple mean comparison testing directly to \proglang{OPMS} objects, while automatically retrieving group determining factors from stored metadata. In the following, the general concept and the structure of the function will be explained using an comparison of PM curves from several strains on one single substrate. Afterwards, several useful features and specific application of arguments will be demonstrated in step-by-step data-processing of other example data. 

\subsubsection[Overview on multiple comparisons]{Overview on multiple comparisons}\label{opm-mcp overview}

To demonstrate the usage of \code{opm\_mcp()} for conducting multiple comparisons of group means directly on \proglang{OPMS} objects \citep{Vaas2013}, we compare four distinct strains, each of which represented by ten replicates of GEN-III microplate measurements. Motivated by an experimental questions that could be addressed with these data, ``Do these four bacteria differ in the mean value of curve parameter A on well G06?'' (see Figure~\ref{fig:xy_plot_mcp}), a first simple multiple comparison of group means can be performed.
...
In our example a Tukey-type contrast matrix, indicating an all-against-all comparison, is applied (a more detailed explanation of contrast matrices, their structure and features, is given in \ref{use-def-comp}). 
The test results in a set of six two-sided pairwise comparisons between all the four strain means (all possible pairwise comparisons).
...
The here used kind of \code{linfct} argument, a numeric vector, refers to the positions of the variables within \code{model} and the first and only term, ``Strain'', is selected.
The names of such a \code{linfct} object indicate the type of contrast used. Per default \code{opm_opt("contrast.type")} would be inserted, if names were missing.

Beside the per default available contrast types (see help page of \code{contrMat()} from package \pkg{multcomp} for details), the specific contrast type \code{Pairs} and an upgraded \code{Dunnett} are implemented and explained in detail below.
...
The filled black circle indicates the point estimator of the difference between the mean of compared groups.
...
\subsubsection[Interpretation of the results]{Interpretation of the results}\label{opm-mcp res-int}
...
In our example, the results from the statistical calculations indicate that all significant differences are even highly significant (p < 0.001).
But, the numerical values of p-values are not correlated with effect sizes indicated only by the distance of CIs from 0. % unclear, now better?

For example, the position of the 95\% CI indicates that the difference between strains DSM 18039 and 429SC1 (i.e, the effect size in the second comparison in Figure~\ref{fig:ci_plot_mcp}) is much larger than between strains DSM 30083\textsuperscript{T} and 429SC1 (smaller effect size in the third comparison), although both tests have highly significant p-values.
...
\subsubsection[Dunnett-type comparison of complete plates]{Dunnett-type comparison of complete plates}\label{comp-plates-dunnett}

When performing PM experiments, one kind of standard analysis is the comparison of all wells/substrates against one single specificly selected well/substrate.
This reference well can be a negative or positive control or, for example, one substrate serving as a standard in a specific chemical group.

To ease this kind of tests \code{opm\_mcp()} contains some special features for ``Dunnett''-type contrasts. 
The following data example is again taken from the first biological replicate included in \code{vaas\_et\_al}, but this time the \textit{Escherichia coli} type-strain subset, measured in ten technical replicates, is selected.
...
The comparison of all wells against the negative control in A01 can now be performed by calling: 

<<label=vaas.e.coli.2, results=hide, width=8>>=
y <- opm_mcp(vaas.e.coli[, , 1:10], output = "mcp", model =  ~ ("Well"), 
  linfct = c(`Dunnett.A01 (Negative Control)` = 1))
@

For matter of convenience, we perform the tests only for the first ten wells.
Generally, the value for the \code{linfct} argument can be constructed by typing \code{Dunnett} plus any sign plus the level name which should serve as the reference group in the contrast set.

If \code{linfct} was stated instead as \code{ linfct = c(Dunnett = 1)} the alphanumerically first level of the addressed factor in \code{model} would serve as the reference group in the contrast set.

Alternatively, the comparison could be computed against the positive control:

<<label=vaas.e.coli.3, results=hide, width=8>>=
y <- opm_mcp(vaas.e.coli[, , 1:10], output = "mcp", model =  ~ ("Well"), 
  linfct = c(`Dunnett.A10 (Positive Control)` = 1))
@

The resulting object is of class \code{glht} and can be plotted as usual using \code{plot()}.

\subsubsection[Pairs-type comparison of complete plates]{Pairs-type comparison of complete plates}\label{comp-plates-Pairs}

When performing PM experiments, another frequently used kind of analysis is the well-wise comparison of strains, treatments, etc. for complete plates.
This can easily be performed by applying the specially designed \code{linfct}-argument \code{Pairs}.

<<label=opm.mcp.linfct.Pairs.1, results=hide, width=8>>=
opm_mcp(vaas_4, model = ~ J(Well + Species), m.type = "aov",
  linfct = c(Pairs = 1), output = "mcp")
@

To determine the groups to be compared, \code{Well} has to be part of the model and joined (by pseudo-function \code{J()}) with at least one other factor extracted from the metadata.
By default the first of some joined metadata entries will be used.
The example shown here computes all 96 tests for one \proglang{OPMS} object comparing the species on each well.

If models are more complex, the names of the metadata entries for which the pairwise-comparisons should be performed can be appended directly to the \code{Pairs} argument.
For instance, by using \code{linfct = c(Pairs.Well = 1)} the metadata name \code{Well} can directly be addressed. 
Similarly, by using \code{linfct = c(Pairs.Species = 1)} \code{Species} will be the pair-determining factor. Thus the result of 

<<label=opm.mcp.linfct.Pairs.2, results=hide, width=8>>=
opm_mcp(vaas_4, model = ~ J(Strain + Species), m.type = "aov",
  linfct = c(Pairs.Species = 1), output = "mcp")
@

is the pairwise comparisons of the strains regarding the two species \textit{Escherichia coli} and \textit{Pseudomonas aeruginosa}, pooled over all wells.
...
For performing even more specific comparisons of interest, the user has the possibility to provide a contrast matrix directly. 
...
Although the user typically expects these wells to be in alphabetical order in an \proglang{OPMS} object, this actually may have been changed by a previous well selection.
...
This object can be returned (omitting the computation of the multiple comparisons) by using \code{output = "data"}:
...
For the special case of the above mentioned \textit{cell-means model} (see Section~\ref{application metadata} and Section~\ref{methods group-means}) it might be necessary to manually merge two or more metadata entries and generate a new one, whose levels (after automated conversion to a factor in a data frame) define the groups of interest:
...
For example, the user might be interested in comparing the data shown in Figure~\ref{fig:opm_mcp_CI_plot_cell-meansI} at the level of groups that may contain different data sets as subgroups. 
