...
This the tutorial of \pkg{opm} in the version of \today{}.
...
%\large
%\textbf{\subsubsection[General principle]{General principle}\label{statcomp general}}
%\normalsize

%% Lea, the subsubsections work well so far.
%% But are the next two paragraphs really necessary here?
%% Shouldn't this already be covered in the methods chapter?
%% Experience with this and other projects indicates that we should avoid redundancies at (almost) all cost. 
...
\begin{itemize}
  \item Tukey-type of comparison within a single well across different plates
  \item Dunnett-type of comparison between wells from the same plate
  \item Pairs-type of comparison of groups defined by specific combinations of metadata entries
  \item User-defined comparisons of interest
\end{itemize}


\large
\textbf{\subsubsection[Tukey-type of comparison within a single well]{Tukey-type of comparison within a single well across different plates}\label{Tukey-type of comparison}}
\normalsize
...
As explained in section \ref{methods group-means}, the \code{opm\_mcp()} methods internally reshape the data into a ``flat'' data frame containing one column for the chosen parameter, one column for the well (substrate) name, another column for the values itself and optionally additional columns for the selected metadata. 
The argument \code{output = "data"} shows the ``flat'' data frame created by \code{opm\_mcp()}. 
...
<<>>=
x <- opm_mcp(vaas.subset, model = ~ Strain, m.type = "aov",
        linfct = c(Tukey = 1), output = "data")
@
...
In our example a Tukey-type contrast matrix is used (a more detailed explanation of contrast matrices, their structure and features, is given at the help page of \code{multcomp::contrMat()}).
The test results in a set of six two-sided pairwise comparisons between all the four strain means (all possible pairs).
The results of the comparison is written into an object termed \code{vaas.subset.mcp}.
...
The here used kind of \code{linfct} argument, a numeric vector, refers to the positions of the variables within \code{model} and here, by using `` \code{1}'' the first and only term, ``Strain'', is selected.
The names of such a \code{linfct} object indicate the type of contrast used, here: `` \code{Tukey}''. Per default \code{opm_opt("contrast.type")} would be inserted, if names were missing.
...
<<label=summary.opm.mcp_1, width=8>>=
...
\large
\textbf{\subsubsection[Interpretation of the results]{Interpretation of the results}\label{opm-mcp res-int}}
\normalsize
...
\subsubsection[Dunnett-type of comparison of complete plates]{Dunnett-type of comparison of complete plates}\label{comp-plates-dunnett}


\large
\textbf{\subsubsection[Dunnett-type of comparison of complete plates]{Dunnett-type of comparison of complete plates}\label{comp-plates-dunnett}}
\normalsize
...
<<label=vaas.e.coli.1, results=hide, width=8>>=
...
<<label=vaas.e.coli.2, results=hide, width=8>>=
...
This substantially eases the choice of the the reference group.

A summary of the resulting data frame, with well A03 chosen to be the reference group, shows the A values from 10 wells of each 10 plates:
\small
<<>>=
summary(opm_mcp(vaas.e.coli[, , 1:10], output = "data", model =  ~ ("Well"), 
        linfct = c(`Dunnett_A03 (D-Maltose)` = 1)))
@
\normalsize

<<label=vaas.e.coli.3, results=hide, width=8>>=
y <- opm_mcp(vaas.e.coli[, , 1:10], output = "mcp", model =  ~ ("Well"), 
             linfct = c(`Dunnett_A03 (D-Maltose)` = 1))
@

The resulting object is of class \code{glht} and can be plotted as usual using \code{plot()}.
Below is the summary (the output of the code is supressed, but see the example above):


\scriptsize
<<label=summary.opm.mcp_2,echo=FALSE, width=8>>=
options(width=200)
...
THE STUFF BELOW HAS BEEN TOLD BY LEA TO BE TRUE; BUT IS SIMPLY WRONG: RECHECK WITH LEA.
If, however, \code{linfct} was stated instead as \code{ linfct = c(Dunnett = 1)} the alphanumerically first level of the addressed factor in \code{model} would serve as the reference group in the contrast set. 
Accordingly, with { linfct = c(Dunnett = 2)} the alphanumerically second level of the addressed factor would serve as the reference group.

\large
\textbf{\subsubsection[Pairs-type comparison of specific wells]{Pairs-type of comparison of groups defined by specific combinations of metadata entries}\label{specific-well-Pairs}}
\normalsize
...
A combination of two species x four well types would yield eight different groups, which are each represented by two plates.
A Tukey-type of comparison (all-against-all) would then result in 28 pairwise comparisons, whereas a Dunnett-type of comparison (one-against-all) would result in seven pairwise comparisons.
However, assume the user is only interested in a specific subset of pairwise comparisons defined by the question ``For each well, is there a difference between the two species?''.
This experimental question results in testing four statistical hypotheses, as there are only four pairwise combinations that fit this question, since for each of the four wells, the two species would be compared. 
Hence, it needs to be ensured that the adjustement for multiplicity (see section~\ref{methods group-means}) is done only for these four statistical hypotheses, despite the presence of eight groups in the data set. 
...
<<label=opm.mcp.linfct.Pairs.1a, width=8>>=
y <- opm_mcp(vaas_4[, ,c( 1:3, 86)], model = ~ J(Well + Species), 
             m.type = "aov", linfct = c(Pairs = 1), output = "mcp")
@
...
<<label=opm.mcp.linfct.Pairs.2, results=hide, width=8>>=
y <- opm_mcp(vaas_4[, , c(1:3, 86)], model = ~ J(Well + Species), m.type = "aov",
             linfct = c(Pairs.Species = 1), output = "mcp")
@
...
Any conversions applied to the \code{model} argument can be shown using \code{output = "model"}. 
This allows the user to control the correct usage of the argument \code{model}.

<<label=opm.mcp.linfct, width=8>>=
opm_mcp(vaas.subset, model = ~ J(Well + Species), m.type = "aov",
...
<<label=opm.mcp.principle_b, results=hide,echo=FALSE, width=8>>=
vaas.subset.mcp <- opm_mcp(vaas.subset, model = ~ Strain, m.type = "aov",
  linfct = c(Tukey = 1))
mcp.summary <- summary(vaas.subset.mcp)
mcp.summary$model$call <- NULL # avoid some unnecessary output
...
SPECIALIST STUFF:
However, \code{linfct} can also be specified more explicitly, as detailed below.
...
WE STILL NEED TO DESCRIBE HOW THE VERSATILE USER CAN MAKE USE OF THIS INFORMATION. Otherwise there might be no sense in addressing this issue here.
