...
\rcsadd{In order to prepare the metadata for being able to set up a \textit{cell-means-model}(see Section~\ref{methods group-means} and ~ref{application group-means} for further details), factors used for defining groups of interest have to be merged. This might be done at this stage when setting up the metadata by using \code{interaction()} for concatenating existing levels of choosen factor-variables.}
<<results=hide, width=10>>=
...
metadata(vaas.subset) <- new.MetaI ~ interaction(Species, Strain)
# all types of metadata 
metadata(vaas.subset) <- new.Meta ~ paste(Species, Strain, sep = ".")
@
...
vaas5.mcp <- opm_mcp(vaas.subset, model = ~ new.Meta, 
  linfct = mcp(new.Meta = "Tukey"))
summary(vaas5.mcp)
@
...
Point estimates and 95\% confidence intervals in Tukey-type comparison of group means for a \textit{cell-means-model} from the \code{vaas.subset} exemplar object.
...
\end{figure}
%\rcscom{
%Lea, we still need an example for the combination of factors using \code{interaction()} or %\code{paste()}.
%}
