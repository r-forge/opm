...
\subsubsection[General idea]{General idea}\label{statcomp general}

The following section will introduce the powerful \code{opm\_mcp()}-functionality. As explained in section \ref{methods group-means} the function internally reshapes the data into a ``flat'' data frame containing one column for the chosen parameter value, one column for the well (substrate) name and optionally additional columns for the selected metadata.
For performing the testing procedure, a model has to be stated that specifies the factor levels that determine the grouping \citep{searle, hothorn}.

The \code{opm\_mcp()} function allows for applying such testing directly to \proglang{OPMS} objects, obtaining these factors from stored metadata. In the following, the general concept and the structure of the function will be explained using an comparison of PM-curves from several strains on one substrate. Afterwards, several useful features and specific application of arguments will be demonstrated in step-by-step data-processing of other example data. 

\subsubsection[opm-mcp-Functionality - Overview]{opm-mcp-Functionality - Overview}\label{opm_mcp overview}
...
In our example a Tukey-type contrast matrix, indicating an all-against-all comparison, is applied (a more detailed explanation of contrast matrices, their structure and features, is given below). 
...
The names of such a \code{linfct} object indicate the type of contrast used. Per default \code{opm_opt("contrast.type")} would be inserted, if names were missing. Beside the per default available contrast-types (see help-page of \code{contrMat()} for details), the specific contrast-type \code{Pairs} and an modification of \code{Dunnett} are implemented and explained in detail below.
...
For the computation of the tests itself, \code{opm_mcp()} uses \code{glht()} from the package \pkg{multcomp} and returns an object of class \code{glht}.
...
\subsubsection[opm-mcp-Results and Interpretation]{opm-mcp-Results and Interpretation}\label{opm_mcp res-int}
...
The point estimator in CI for differences of means represents the computed difference of the considered group means and, analogously, the size of the interval tells about the reliability of this difference. If such the 95\% CI for differences of means includes zero (dashed vertical lines in Figure~\ref{fig:ci_plot_mcp}) there is no significant difference between the group means. Conversely, if zero is not included, a statistically significant difference is indicated.
...
For explanation of the graphical representation of the CIs, we have a look at the last comparison in Figure~\ref{fig:ci_plot_mcp}. The comparison of mean A value from strain DSM 30083\textsuperscript{T} minus the mean of A values of strain DSM 18039 results 26.615 units, giving the point estimator of this difference which is plotted at position 26.615 on the x-axis.
That is, on average the A values from well G06 and Strain DSM 30083\textsuperscript{T} are 26.615 units larger than the A values of strain DSM 18039. (The detailed numeric outcome would be obtained using \code{mcp.summary()} or \code{confint(vaas.subset.mcp)}).

Additionally, each point estimator for the difference of means comes with an 95\% CI providing information about the statistical significance of the test, the effect size and the variability of the mean differences. They are plotted as usual.

In our example, the results from the statistical calculations indicate that all significant differences are also highly significant (p < 0.001). However, please keep in mind, that their numerical values are not correlated with effect sizes: The position of the 95\% CI indicates that the difference between strains DSM 18039 and 429SC1 (i.e, the effect size in the second comparison in Figure~\ref{fig:ci_plot_mcp}) is much larger than between strains DSM 30083\textsuperscript{T} and 429SC1 (smaller effect size in the third comparison).
For a meaningful biological interpretation of the results it is therefore highly recommended to consider the effect size rather than only taking into account the p values.

\subsubsection[Comparison of complete plates - Dunnett]{Comparison of complete plates - Dunnett}\label{comp plates dunnett}

When performing PM-experiments, one standard analysis is the comparison of the wells against one other group. This other group can be a negativ or positive control or, for example, one substrate serving as a standard in a specific chemical group. 

To perform the testing on such a set of comparison the \code{opm\_mcp()} comes with some special features for the Dunnett-type contrast. 
The necessary data-subset are again taken from the first biological replicate included in \code{vaas\_et\_al}, but this time the \textit{Escherichia coli} strain vaas.sub.plate, measured in ten technical replicates is selected.

<<label=data.vaas.subset, results=hide, width=8>>=
vaas.sub.plate <- subset(vaas_et_al, 
  list(Experiment = "First replicate", Strain = "DSM30083T"))
@

The comparison of all wells against the A01 negative control can now be performed by calling: 

<<label=data.vaas.subset, results=hide, width=8>>=
y <- opm_mcp(vaas.sub.plate[, , 1:10], output = "mcp", model =  ~ ("Well"), 
  linfct = c(`Dunnett.A01 (Negative Control)` = 1))
@

For matter of convenience, we perform the tests only for the first ten wells. Generally, the value for the \code{linfct}-argument can be constructed by typing \code{Dunnett} plus any sign plus the level name, which should serve as the reference group in the contrast-set.

If \code{linfct} would be stated only as \code{ linfct = c(Dunnett = 1)} the alpha-numerically first level of the addressed factor in \code{model} served as the reference group in the contrast-set.
...
\subsubsection[Comparison of complete plates - Pairs]{Comparison of complete plates - Pairs}\label{comp plates Pairs}

When performing PM-experiments, another usual analysis approach is the well-wise comparison of strains, treatments, etc. for complete plates. This can be easily performed by application of the specially designed \code{linfct}-argument \code{Pairs}.

<<label=opm.mcp.linfct.Pairs, results=hide, width=8>>=
opm_mcp(vaas_4, model = ~ J(Well + Species), m.type = "aov",
  linfct = c(Pairs = 1), output = "mcp")
@

In order to determine the groups to be compared, \code{Well} has to be part of the model and joined (by function \code{J()}) with at least one other metadata-factor. The here given example computes all 96 tests for one OPMS object comparing the species on each well.

If models are more complex, the \code{Pairs}-argument offers the flexibility to adress directly the metadata for which the pairwise-comparisons should be performed. Per default the first given metadata-name in the \code{model}-argument will be used. By using \code{linfct = c(Pairs.Well = 1)} the metadata-name \code{Well} can be adressed directly. 
...
gives the pairwise comparisons of the strains regarding the two species \textit{Escherichia coli} and \textit{Pseudomonas aeruginosa}. When dealing with more complex models also keep in mind the flexibility offered by the numeric vector in \code{linfct}, which refers to the positions of the variables within \code{model}.
