...
{\color{red} TODO Lea Explain the normalisation method from extract()
}
...
Since most addressed experimental questions of an PM-approach desire to statistically compare not only single curves, but distinct groups of curves, the function \code{opm_mcp} provides fuctionality for performance of simultaneous multiple comparisons of group means by internally calling \code{glht} from \pkg{multcomp}. Using available metadata and/or the substrate-names the user is enabled to define groups of interest, set up a model of choice and perform multiple comparison of group means on individually specified contrasts \citep{bretz, hsu}. As not differentially specified, comparisons for the different curve parameters are performed separately.

TODO: describe here, why multiple comparisons of means, why not ANOVA, why multcomp and nothing else?

The function internally reshapes the data into a 'flat' data frame containing one column for the chosen parameter-value, one column for the well-name and optionally additional columns for the selected metadata. For performance of the testing procedure, a model has to be determined thereby specifying the factor's levels which determine the grouping \citep{searle, hothorn}.

TODO Lea Introduce functionality of \code{opm\_mcp()} and \code{extract()}; explain the implementation of how factors are merged to produce a pseudo-one-way-layout \citep{schaarschmidt}.

Vorschlaege johannes:
(1) Wir sollten erlaeutern, warum der User eventuell motiviert sein kuennte, multiple Vergleiche zu machen. Viele user sind eventuell nur mit der Idee von paarweisen Vergleichen vertraut und kaemen vielleicht von selber nicht auf die Idee, etwas multiples zu machen. Solche user wuerden wir dadurch konkret auf bestimmte Analysemoeglichkeiten hinweisen, auf die er selber nicht kaeme.
(2) Wir sollten erlaeutern, warum wir gerade multcomp nehmen. Was sind die Vorteile von multcomp, im Gegensatz zu anderen Vorgehensweisen, wie z.B. der doch viel bekannteren Anova und dann z.B. Bonferroni-Korrektur (solche Argumente koennen kommen: Nehmt doch das Bekannte)? Auch wenn so ein Absatz einigen sehr trivial erscheinen mag, wir muessen damit rechnen, dass viele Leser der Vignette von so etwas noch gar keine Ahnung haben, daher ist so ein kleiner Textbook-Absatz schon notwendig. Wir sollten die user da gleich richtig "einnorden".
...
(4) Und dann kauen wir ein paar Beispiele durch.
}
