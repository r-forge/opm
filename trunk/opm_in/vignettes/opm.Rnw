...
The package includes, beside 95\% confidence plots and enhanced heat-map graphics for visual comparisons of the estimated curve parameters, functionality for multiple comparison of group-means.
...
Hier absatz ueber statistische vergleiche einfuegen
...
Point also on Fig. 2. Explain the right part of the figure.
...
Explain the normalisation-method from \code{group\_CI90}
...
Introduce functionality of \code{opm\_mcp()} and \code{group\_CI()}
...
here the application of \code{opm\_mcp()} with example and output-graphics.
