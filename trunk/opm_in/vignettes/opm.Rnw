...
Missing entries will be filled with \code{NA} values, and nested metadata entries will yield data-frame columns of the mode ``list''.
...
In order to prepare the metadata for being able to set up a \textit{cell-means-model}(see Section~\ref{methods group-means} and ~ref{application group-means} for further details), factors used for defining groups of interest have to be merged. 
This might be done already at the initial step when setting up the metadata data frame \textit{before} including the metadata to any \proglang{OPM(A)(D)(S)} object using \code{include_metadata()}. 
Here, the function \code{interaction()} is used in order to concatenate existing levels of choosen factor-variables. 
As a result, two metadata entries are merged to a single one.

<<results=hide, width=10>>=
metadata.example$Colour.Position <- interaction(metadata.example$Colour, 
  metadata.example$Position, sep = ".", drop = TRUE)
@
 
However, most of metadata might not be a factor-variable and/or when initially setting up the metadata-table the user might not have yet decided which groups are finally of interest for comparisons and thus, which variable(s) should be used for group-definition. 
Hence, the user might wish to combine metadata for group-definitions \textit{after} the metadata have been included to any \proglang{OPM(A)(D)(S)} object by \code{include_metadata()}.
In order to avoid to start from the beginning by manipulating a metadata dataframe followed by including it to the \proglang{OPM(A)(D)(S)} object, the more flexible framework of employing formulas and expressions for modification of metadata together with \code{paste()} enables here the subsequent manipulation of metadata already present in \proglang{OPM(A)(D)(S)} objects.
The following code operates directly in the \proglang{OPM(A)(D)(S)} object. (Note that previously the key has been changed from ``Colour'' to `Colony colour`)
...
As result, a new metadata entry with the key ``Col.Int'' is created, which contains the combination of the metadata values addressed in \code{paste()}. 

\rcscom{
Markus, I can not use the same metadata entries, as the entry "Position" gets lost in the example.opm during the series of codes in the vignette
}
...
For the special case of the above mentioned \textit{cell-means-model} (see Section~\ref{application metadata} and \ref{methods group-means}) it might be necessary to merge two or more metadata-variable and generate a single factor-variable which levels define the groups of interest. 
For metadata which are represented by factor-variables \code{interaction()} is directly applicable, while \code{paste()} provides a framework applicable on all types of metadata-variables.

<<results=hide, width=8>>=
vaas.G06 <- vaas_et_al[, , "G06"]
metadata(vaas.G06)[114]
# for factor-variables
metadata(vaas.G06) <- Str.Exp ~ interaction(Strain, Experiment)
# all types of metadata 
metadata(vaas.G06) <- Str.Exp ~ paste(Strain, Experiment, sep = ".")
metadata(vaas.G06)[114]
@

With the newly generated metadata-variable \code{Str.Exp} comprehending \code{Strain} and \code{Experiment}, the multiple comparison procedure is straightforwardly processable:

<<results=hide, widht=8>>=
vaas5.mcp <- opm_mcp(vaas.G06, model = ~ Str.Exp, 
  linfct = mcp(Str.Exp = "Dunnett"))
@

The corresponding visualisation of the CI is shown in Figure~\ref{fig:opm_mcp_CI_plot_cell-meansI}.

\begin{figure}
\scalebox{0.9}{
<<fig=TRUE, echo=FALSE, results=hide, width=8, height=4>>=
 old.mar <- par(mar = c(3, 22, 3, 1)) # adapt plotting settings
 plot(summary(vaas5.mcp))
 par(old.mar) # reset plotting settings
@
}
\caption{
\label{fig:opm_mcp_CI_plot_cell-meansI}
Point estimates and 95\% confidence intervals in Dunnett-type comparison of group means for a \textit{cell-means-model} from the \code{vaas.G06} exemplar object.
Analogously to Figure~\ref{fig:opm_mcp_CI_plotII}, the picture was obtained by running \code{opm\_mcp()} and then the plotting function for the resulting object.
Compare also with Figure~\ref{fig:ci_plot_mcp}, where details on the axis annotation are given.
}
\end{figure}

Finally, besides the multiple comparison of single group means as described above, it is also possible to compare averages of subgroups versus single groups or averages of other subgroups.
For example, the user may be interested to compare the data shown in Figure~\ref{fig:opm_mcp_CI_plot_cell-meansI} at the level of groups that may contain different data sets as subgroups. 
When building a contrast-matrix, keep in mind that the levels of the model-defining factor needs to frame the columns of the contrast-matrix in the same order as they occur in the underlying dataframe. 
It is therefore necessary to check level-names and order of the group defining factor.
Further, the user is free to choose the values for coefficients except. 
In the example below, the values 0, -1/4 , 1/4, and 1 are used. 
However, the sum of the coefficients of one contrast must to be zero. 

<<results=hide, width=8>>=
# provide data for multiple comparison
data <- opm_mcp(vaas.G06, model = ~ Str.Exp, 
  linfct = mcp(Str.Exp = "Dunnett"), output = "data")

# check level-names and order of the group defining factor
levels(data$Str.Exp)

# build contrast-matrix
contr <- rbind(
  "First rep - Second rep" = c(1/4, -1/4 , 1/4 , -1/4 , 1/4, -1/4, 0, 1/4, -1/4),
  "First rep - TimeS" = c(1/4, 0, 1/4 , 0, 1/4, 0, -1, 1/4, 0), 
  "DSM1707.Fr - Second rep" = c(0, -1/4 , 1 , -1/4 , 0, -1/4, 0, 0, -1/4))

# assign level-names as column-names and check entries 
colnames(contr) <- levels(data$Str.Exp) 
contr

# compute multiple-comparison based on the defined contrast-matrix
...
<<fig=TRUE, echo=FALSE, results=hide, width=8, height=3>>=
 ...
 plot(summary(vaas6.mcp))
 par(old.mar) # reset plotting settings
@
}
\caption{
\label{fig:opm_mcp_CI_plot_cell-meansII}
Point estimates and 95\% confidence intervals in user defined comparison of group means for a \textit{cell-means-model} from the \code{vaas.G06} exemplar object.
Analogously to Figure~\ref{fig:opm_mcp_CI_plotII}, the picture was obtained by running \code{opm\_mcp()} and then the plotting function for the resulting object.
Compare also with Figure~\ref{fig:ci_plot_mcp}, where details on the axis annotation are given.
}
\end{figure}

The corresponding visualisation of this highly user-defined contrast-matrix is shown in Figure~\ref{fig:opm_mcp_CI_plot_cell-meansII}.
