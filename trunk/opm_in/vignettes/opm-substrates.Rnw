...
In many experimental approaches, the identification of the pathway(s) of interest are subject of research.
...
%
<<label=prep.mcp.dat, eval=FALSE>>=
coli.comp.full # result from mcp
coli.anno.full # annotation-vector mir NAs
coli.anno.full.f # annotation-vector without NAs
@
% compute one mcp for the group of carbohydrates and one for the non-carbohydrates
% use find_positions, something like find_positions(rownames(coli.anno), plate_type(coli.sub)) to select the wells to be compared in the [] 
% how do i merge the information from coli.anno.full with coli.sub to be able to compute the necessary mcps?
% first with select all the rownames of entries which have a 1 in column Carbohydrates
% then use these names with find positions and put in mcp
% do the same with 0 in column carbohydrates


%mcp.dat <- coli.anno.full[, c("Value","Carbohydrates") ]
%mcp.dat$Carbohydrates <- as.factor(mcp.dat$Carbohydrates)
%mcp.dat$Well <- as.factor(rownames(mcp.dat))
%head(mcp.dat)
%mcp.data <- data.frame(mcp.dat[, 2], Parameter = gl(1, 75, labels = "A"), 
%                       mcp.dat[, c(1,3)])
<<label=carb-comp>>=
