...
old.par <- par(mar = c(4, 8, 0.1, 0.1)) # adapt plotting settings
plot(stab, type = "maxsel", main = "", np = 20)
par(old.par) # reset plotting settings
@
}
...
## to speed up computation on linux systems set number of
## cores in cvrisk: e.g. mc.cores = 25
stab2 <- stabsel(mod2, cutoff = 0.75, PFER = 1.5, assumption = "unimodal")
@
...
plot(stab2, type = "maxsel", main = "", np = 20, labels = nms)
par(old.par) # reset plotting settings
@
...
Therefore we first determine the optimal stopping iteration of the boosting algorithm:
