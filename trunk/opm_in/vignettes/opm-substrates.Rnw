...
Point estimates and 95\% confidence intervals for the differences of means between the two \textit{E. coli} strains for the subset of substrates represented in the Galactose pathway map. The blue dashed line determines an choosen minimal effect size for analysis later on. 
}
\end{figure}
\end{center}
...
anno.k12.mat <- annotated(e.coli.k12, how = "value")
anno.type.mat <- annotated(e.coli.type, how = "value")

# make a dataframe 
df.12 <- data.frame(anno.k12.mat)
df.12$strain <- gl(1, 96, labels = "k12")
str(df.12)
colnames(df.12)

df.type <- data.frame(anno.type.mat)
df.type$strain <- gl(1, 96, labels = "type")
str(df.type)
colnames(df.type)

df.coli <- rbind(df.12, df.type)
str(df.coli)
cdf.coli <- na.omit(df.coli)

library(randomForest)
coli.rf <- randomForest(Value ~ ., data = cdf.coli, importance=TRUE,
                        proximity=TRUE)
varImpPlot(coli.rf)
@
