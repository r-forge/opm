...
In addition to visual inspection or statistical comparative analyses of PM data, as described in the main tutorial, users might be interested in specific information on the substrates used in PM assays.
...
\begin{figure}
 \centering
        ...
        Galactose metabolism pathway maps for the two \textit{E. coli} strains K12 and DSM 30083\textsuperscript{T} showing the mean maximum height on 13 substrates. 
        The aggregated and discretized measurement data are represented by according colours and mapped on the corresponding substrates (circles) in the graph. 
        }
\end{figure}
...
The mapping of significant differences between the two strains on the Galactose pathway is shown in Figure~\ref{fig:pathview-native-sig-differences}. One can directly compare the colour-mapping for substrates exhibit an statistically significant differences between the investigated strains (Figure ~\ref{fig:pathview-native-sig-differences-1a}) and the output of investigation differences according a given effect size. In the example substrates which show that DSM18039 has an at least 150 Units higher maximum height value are choosen (Figure ~\ref{fig:pathview-native-sig-differences-1b}).
...
#
# differences regarding an effect-size of at least 150 units upwards
#
cat.comp.anno.t <- annotated(colicomp, output = "'150", lmap = 1:3)
cat.coli.comp.t <- opm_path(cpd.data = cat.comp.anno.t, 
  species = "ko", out.suffix = "cat-coli-comp-t", pathway.id = "00052",
  limit = list(gene = 2, cpd = c(1, 3)), bins = list(gene = 0.5, cpd = 3))
@
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=14cm,type=png,ext=.png,read=.png]{ko00052.cat-coli-comp}}
                \caption{Descretized respiration differences between \textit{E. coli} K12 and DSM 30083\textsuperscript{T} regarding the maximum-height values mapped on the Galactose pathway.}
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=14cm,type=png,ext=.png,read=.png]{ko00052.cat-coli-comp-t}}
                \caption{Descretized respiration differences between \textit{E. coli} K12 and DSM 30083\textsuperscript{T} with minimal effect size of 150 units regarding the maximum-height values mapped on the Galactose pathway.}
                \label{fig:pathview-native-sig-differences-1b}
        \end{subfigure}
        \caption{
        \label{fig:pathview-native-sig-differences}
        Significant respiration differences between \textit{E. coli} K12 and DSM 30083\textsuperscript{T} regarding the maximum-height values mapped the Galactose pathway.
