...
<<label=pathview-wrapper-outob, fig=TRUE>>=


# results from mcp
anno.colicomp <- annotated(colicomp, how = "value")

# make a dataframe 
anno.comp.df <- data.frame(anno.colicomp)
str(anno.comp.df)
# randomForest
library(randomForest)
# build model
colicomp.rf <- randomForest(Value ~ ., data = anno.comp.df, importance=TRUE,
                        proximity=TRUE)
# plot of important variables
varImpPlot(colicomp.rf)
@
