...
For just downloading information from \ac{KEGG} (see Section~\ref{plate-information}), install the \proglang{Bioconductor} package \pkg{KEGGREST}.
...
For substrates such as ``Positive Control'' or ``pH 5'' no \ac{KEGG} ID is available, which results in \code{NA} values in the vector.
...
Please note that the underlying test seeks for differences and thus the results always have to be interpreted regarding the occurrence of differences: Mainly ``insignificantly different'' does not mean ``significantly equal''.
...
In the case of \ac{KEGG}, a pathway map is described as ``a molecular interaction/reaction network diagram represented in terms of the \ac{KEGG} Orthology (KO) groups'' (see \url{http://www.genome.jp/kegg/kegg3a.html} for further details).
...
Note that the \code{species} arguments offers the possibility to use species-specific genome information available in the \ac{KEGG} directory; see above for the letter codes.
...
\code{NA}s indicate that a substrate has no \ac{KEGG} ID, as for example well A01 containing the (pseudo-)substrate ``Negative Control''.
...
old.mar <- par(mar = c(3, 25, 3, 0.5)) # adapt plotting settings
...
\begin{figure}
\scalebox{1.225}{
\includegraphics[width=10cm,type=png,ext=.png,read=.png,page=1]{ko00052.coli.comp.ko}
}
...
coli.graphvizmap.k12 <- opm_path(cpd.data = anno.k12, species = "ko", afactor = 1500,
...
\begin{figure}
\scalebox{1.225}{
\includegraphics[width=10cm,type=pdf,ext=.pdf,read=.pdf,page=1]{ko00052.graphvizk12.ko}
}
...
In the next coding example, we prepare the data matrix for difference between the two \textit{E. coli} strains (as computed in section ~\ref{pathview-group-diffs}) and add potential explanatory variables from \ac{KEGG}. 
Please refer to the manual and Section~\ref{annotated-input} for explanation of usage of \code{annotated} with argument \code{how = "values"}.
...
# compute the differences between the two strains for all substrates  
coli.comp.full <- opm_mcp(coli.sub[, , ], 
  output = "mcp", model = ~ J(Well + Strain), linfct = c(Pairs = 1))
# provide annotated vector for the differences
coli.anno.full <- annotated(coli.comp.full, how = "value")
@

The matrices have to be reduced to the non-\code{NA} rows, i.e. substrates that are not included in \ac{KEGG} have to be removed.
It also needs to be converted to a data frame before the \pkg{randomForest} package \citep{Liaw2002} can be applied.

<<label=e-coli-dat>>=
coli.anno.full.f <- as.data.frame(coli.anno.full[complete.cases(coli.anno.full), ])
@

In the random forest approach, we use a regression to find out, which substrate features, represented by their occurrence in maps or the substrate categories, explain the observed differences between the two \textit{E. coli} strains. 

<<label=e-coli-rf>>=
suppressPackageStartupMessages(library("randomForest"))
coli.comp.full.rf <- randomForest(x = coli.anno.full.f[, -1], y = coli.anno.full.f[, 1],
...
varImpPlot(coli.comp.full.rf, main = "Difference Type Strain vs. K12")
@

\begin{center}
\begin{figure}
\scalebox{1.25}{
<<label=random-forest-plot, fig=TRUE, echo=FALSE, width=10, height=7.5>>=
varImpPlot(coli.comp.full.rf, main = "Difference Type Strain vs. K12")
@
}
\caption{
...
That is, one of the strains performs stronger than the other particularly regarding carbohydrate utilisation.
...
For demonstration purposes we show now the pooled comparison between substrates from category  ``Carbohydrates'' and the remaining substrates.

First, the positions of the substrates belonging to ``Carbohydrates'' and others are determined and afterwards stored in two different vectors. 
<<label=prep.mcp.dat>>=
dat <- data.frame(coli.anno.full)
carb.sub <- subset(dat, Carbohydrates == 1)
non.carb <- subset(dat, Carbohydrates == 0)
carb.pos <- find_positions(rownames(carb.sub), plate_type(coli.sub))
non.c.pos <- find_positions(rownames(non.carb), plate_type(coli.sub))
@

Now we can setup the contrast matrix needed for the specified computation of the multiple comparisons between the two strains for the two subsets of substrates.
For matter of convenience we use functionality from \code{opm_mcp} to provide a helper-contrast-matrix which serves to extract the needed column-names and indicates number of columns for the final contrast matrix.

<<label=carb-comp-contr-mat>>=
contr.carb <- opm_mcp(coli.sub[, , c(carb.pos)], linfct = c(Dunnett = 1),
  model = ~ J(Well + Strain), output = "contrast")
contr.n.carb <- opm_mcp(coli.sub[, , c(non.c.pos)], linfct = c(Dunnett = 1),
  model = ~ J(Well + Strain), output = "contrast")
@  

The necessary numeric entries are generated using the columns-dimension of the helper-contrast-matrix.

<<label=carb-comp-contr-entry>>=
carb.names <- colnames(contr.carb$Well.Strain)
carb.vec <- rep(c(1/(dim(contr.carb$Well.Strain)[2]/2),
  -1/(dim(contr.carb$Well.Strain)[2]/2)), 
  times = (dim(contr.carb$Well.Strain)[2]/2))
carb.vec.fill <- rep(0, times = dim(contr.n.carb$Well.Strain)[2])
#
n.carb.names <- colnames(contr.n.carb$Well.Strain)
n.carb.vec <- rep(c(1/(dim(contr.n.carb$Well.Strain)[2]/2),
  -1/(dim(contr.n.carb$Well.Strain)[2]/2)), 
  times = dim(contr.n.carb$Well.Strain)[2]/2)
n.carb.vec.fill <- rep(0, times = dim(contr.carb$Well.Strain)[2])
@

The final contrast matrix is set up by concatenating the numeric vectors, stating labels for which groups are compared and adding of the column-names. 

<<label=carb-comp-contr-finish>>=
contr <- rbind(
  "Carbohydrates/DSM18039 - Carbohydrates/DSM30083T" = c(carb.vec, carb.vec.fill),
  "nonCarb/DSM18039 - nonCarb/DSM30083T" = c(n.carb.vec.fill,n.carb.vec))
#      
colnames(contr) <- c(carb.names,n.carb.names)              
@

The contrastmatrix \code{contr} serves now as direct input in \code{opm\_mcp}, as follows.
See also the manual of \code{opm\_mcp} for further details.
<<label=carb-comp-test>>=
carb.test <- opm_mcp(coli.sub[, , c(carb.pos,non.c.pos )], linfct = contr,
  m.type = "lm",  model = ~ J(Well +Strain), output = "mcp")
@
...
\begin{figure}
\scalebox{1.0}{
<<label=carb-comp-test, fig=TRUE, echo=FALSE, width=10, height=3>>=
par(mar = c(3,25,4,3))
plot(carb.test, xlim = c(-170, 20))
@
}
\caption{
\label{fig:carb-comp-test}
Point estimates and 95\% confidence intervals for the comparison of the two \textit{E. coli} strains pooled over substrates classified as ``Carbohydrates'' and non Carbohydrates (``nonCarb'').
The Difference between the two strains is much larger on the Carbohydrate-substrates than on the remaining ones.
}
\end{figure}
\end{center}
...
%TODO Should we confirm this result with opm_mcp?
%
% we then maybe need better selection mechanisms from the matrix/data frame as output by annotated()
% let me know if something looks complicated
%
% but we already have:
%R> find_positions(rownames(coli.anno), plate_type(coli.sub))
%
% an opm_mcp example comparing the differences between the two strains regarding
% carbohydrates vs. non-carbohydrates could be interesting to conclude this section
%
% in addition to the content, it would also provide examples for dealing with the output data
...
Note that the approach used above is suboptimal because mean differences between metadata-defined groups have entered  the model as dependent variable.
