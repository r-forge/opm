...
A detailed description of the OmniLog\textregistered \ Phenotype MicroArray (PM) system, its measuring procedure and data characteristics are found in the vignette ``\pkg{opm}: An R Package for Analyzing OmniLog\textregistered \ Phenotype MicroArray Data'' (called ``main tutorial'' in the following).
...
In addition to visual inspection or statistical comparative analyzes of PM data, as described in the main tutorial, users might be interested in specific information on the substrates used in PM assays.
The \pkg{opm} package contains a large variety of additional data on PM substrates.
Beside methods for assessing this information directly, this tutorial introduces strategies for visualizing the measured PM results by mapping them on pathway maps.
Furthermore, analysis methods are described for modelling the effect of substrate features on the PM results and thus, e.g., for the identification of those pathways that are particularly suitable for visualizing the PM results in pathway graphs.
...
\section{Visualization of PM information within pathway maps}\label{vis-pathway}
...
The mapping itself works by using information produced by \pkg{opm} for a color coding of the nodes (here, representing the substrates) within those maps, as can be done similarly with several other types of ``OMICS'' data such as transcriptomics or proteomics data.
...
In the following example, first an ``opm\_glht'' object is generated from the \code{vaas\_4} exemplar object by performing a Dunnett-type multiple comparison of the selected 13 wells against well A05 as control group.

<<label=annotated-opm_glht-plot>>=
x <- opm_mcp(vaas_4[, , 1:15], 
...
\subsection{Visualization in pathway maps using pathview}\label{pathview}

\subsubsection{Visualization of group means in pathway maps}\label{pathview-group}
...
Afterward we create the annotated vectors containing the average maximum curve heights for the two groups separately:
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.k12.ko}}
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.type.ko}}
        ...
        The aggregated measurement data are represented by according colors and mapped on the corresponding substrates (circles) in the graph. 
   		}
\end{figure}
...
This works because the underlying \proglang{OPMX} object contains discretized data.
...
anno.k12.bin <- annotated(e.coli.k12, output = param_names("disc.name"),
  lmap = 1:3)
anno.type.bin <- annotated(e.coli.type, output = param_names("disc.name"),
  lmap = 1:3)
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.k12.ko.bin}}
                \caption{Respiratory data (mean maximum height, discretized) of \textit{Escherichia coli} strain K12 mapped on the KEGG Galactose pathway.}
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.type.ko.bin}}
                \caption{Respiratory data (mean maximum height, discretized) of \textit{Escherichia coli} type strain DSM 30083\textsuperscript{T} mapped on the KEGG Galactose pathway.}
                \label{fig:pathview-cat-sep-strains-1b}
        \end{subfigure}
        \caption{
        \label{fig:pathview-cat-sep-strains}
        Galactose metabolism pathway maps for the two \textit{E. coli} strains K12 and DSM 30083\textsuperscript{T} showing the mean maximum height on 13 substrates. 
        The aggregated and discretized measurement data are represented by according colors and mapped on the corresponding substrates (circles) in the graph. 
        }
\end{figure}
...
This is no wonder because the PM plates are not arranged according to their affiliation to KEGG pathways.
It often makes sense to restrict the considered substrates beforehand if the pathway of interest is already known.
This particularly saves running time in the calls to \code{opm\_mcp} and the \code{annotated} method for ``opm\_glht'' objects.

When using the option \code{how = "value"}, \code{annotated} yields a numeric matrix with substrate names as row names and pathway identifiers as column names.
...
\code{NA}s indicate that a substrate has no KEGG ID, as for example well A01 containing the (pseudo-)substrate ``Negative Control''.
...
\subsubsection{Visualization of differences of group means in pathway maps}\label{pathview-group-diffs}
...
colicomp <- opm_mcp(e.coli.sub[, , e.subs.pos], 
...
\begin{figure}
\scalebox{1.1}{
<<label=e-coli-comp-ci-figure, fig=TRUE, echo=FALSE, width=12>>=
old.mar <- par(mar = c(3, 8, 3, 2))
par(mar = c(3, 20, 3, 2)) # adapt plotting settings
...
Thus the full power of the functions from the \pkg{multcomp} package underlying \code{opm\_mcp} (see the main tutorial and the manual for details) is available when visualizing PM data in pathway graphs.
...
Because we chose the ``upwards'' running mode, up to three colors are used in the map, indicating whether the performance difference is significantly larger than the minimum effect size, insignificantly different from the minimum effect size, or significantly smaller than the minimum effect size.
...
cat.coli.comp.0 <- opm_path(
  cpd.data = annotated(colicomp, output = "upwards", lmap = 1:3), 
...
cat.coli.comp.150 <- opm_path(
  cpd.data = annotated(colicomp, output = "'150", lmap = 1:3), 
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.cat-coli-comp-0}}
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                \centering
                \fbox{\includegraphics[width=13cm,type=png,ext=.png,read=.png]{ko00052.cat-coli-comp-150}}
        ...
        Indicators of the significance of the respiration differences between \textit{E. coli} K12 and \textit{E. coli} DSM 30083\textsuperscript{T} regarding the maximum-height values mapped on the Galactose pathway.
Blue, significantly larger than the chosen minimum effect size; gray, insignificantly different from the chosen minimum effect size; yellow, significantly smaller than the chosen minimum effect size.
...
\subsubsection{Visualization of pathway maps in de-novo layout}\label{pathview-denovo-layout}

In addition to the native KEGG visualization, \code{pathview} can use a \textit{de-novo} visualisation approach.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% END FROZEN PART
%% DO NOT MODIFY ANYTHING ABOVE THIS LINE WITHOUT NEGOTIATING WITH MG
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In the examples detailed above, a variety of such objects have already been generated, e.g., \code{e.coli.map.k12} or the \code{e.coli.map.types} (both described in Section~\ref{pathview-group} and visualized in Figure~\ref{fig:pathview-native-sep-strains} therein).

Next, we show how to produce graphics from these objects:

<<label=pathview-plot-graphviz>>=
e.coli.graphvizmap.k12 <- opm_path(cpd.data = anno.k12, species = "ko", 
 kegg.native = FALSE, out.suffix = "graphvizk12.ko", pathway.id = "00052",
 sign.pos = "bottomleft", key.pos = "bottomright")
@
...
Respiratory data (mean maximum height) of \textit{Escherichia coli} strain K12 mapped on the KEGG Galactose pathway and rendered using graphviz layout engine.
Compare also Figure~\ref{fig:1a-pathview-native-sep-strains}, which shows the native KEGG rendering of these data.
}
\end{figure}

\rcsmark{TODO}
Description of the poor options for tuning the font sizes or size of symbols in the graphic are needed, similarly, colour-arguments and the scaling need to be exoplained properly.

% this part goes into the paragraph, where it is described, that there is no fixed order of the different described analysis steps:
...
In many cases, the pathways of interest are not known in advance.
The information accessible \textit{via} \code{annotated} can be used in conjunction with suitable statistical procedures to automatically determine those features of the substrates that best correspond to a certain experimental outcome.
Such results can be used as a starting point when searching for a causal explanation of the given \pkg{opm} results.
They can also be used to simply determine the pathways to be used when drawing the \pkg{opm} results in a graph, as detailed in the previous sections.
\rcsmark{TODO}

In the next example, we use the \pkg{randomForest} package \citep{Liaw2002} for determining the substrate features that best predict the differences between the two \textit{E. coli} strains and depict these features within a ``Variable Importance Plot''.

<<label=randomforest-figure, fig=FALSE, eval=FALSE>>=
anno.colicomp <- as.data.frame(annotated(colicomp, how = "value"))
suppressPackageStartupMessages(library("randomForest"))
colicomp.rf <- randomForest(Value ~ ., data = anno.colicomp,
  importance = TRUE, proximity = TRUE)
varImpPlot(colicomp.rf, main = NULL)
@

\begin{center}
\begin{figure}
\scalebox{1.1}{
<<label=random-forest-plot, fig=TRUE, echo=FALSE, width=12>>=
anno.colicomp <- as.data.frame(annotated(colicomp, how = "value"))
suppressPackageStartupMessages(library("randomForest"))
colicomp.rf <- randomForest(Value ~ ., data = anno.colicomp,
  importance = TRUE, proximity = TRUE)
...
\caption{
\label{fig:rf-plot}
\rcsmark{TODO} Here some informative description of the variable-importancy-plot.
}
\end{figure}
\end{center}


The output can be interpreted a follows. \rcsmark{TODO}

%TODO Carbohydrates (and their subcategories) are nicely shown here, but isn't his just due to the preselection of substrates?
% How time consuming is it to analyze all substrates of that plate?
...
We are grateful to Barry Bochner (BIOLOG Inc.) for providing substrate and plate information on Phenotype MicroArray assays.
The integration of missing OmniLog\textregistered \ substrates into ChEBI by the ChEBI staff is gratefully acknowledged.
We thank Weijun Luo for providing hints regarding the use of the \pkg{pathview} package.
