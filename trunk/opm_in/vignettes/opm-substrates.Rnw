...
With \code{annotated} used with \code{how = "value"} the numeric matrix including the substrate names as row names and first column indicating the mean of chosen computed values is provided.
...
%TODO 
% wenn man value als y-variable nimmt
% But this is now regression, not classification as originally intended.
% E.g. there is no confusion matrix.
% IMHO this is appropriate because we have a quantitative response.
% Please check this.
% If OK, explain some details of this analysis here but not too much.
% Instead, include some references if possible.
...
First, the positions of the substrates belonging to ``Carbohydrates'' and others are determined:
<<label=prep.mcp.dat>>=
...
carb.vec <- rep(c(1/22,-1/22), times = 22)
carb.vec.fill <- rep(0, times = 112)
#
n.carb.names <- colnames(contr.n.carb$Well.Strain)
n.carb.vec <- rep(c(1/56,-1/56), times = 56)
n.carb.vec.fill <- rep(0, times = 44)
#
contr <- rbind("Carbohydrates/DSM18039 - Carbohydrates/DSM30083T" = c(carb.vec, carb.vec.fill),
               "nonCarb/DSM18039 - nonCarb/DSM30083T" = c(n.carb.vec.fill,n.carb.vec))
#      
colnames(contr) <- c(carb.names,n.carb.names)              
@

The setup contrastmatrix \code{contr} serves now as direct input in \code{opm\_mcp}, as follows.
...
\begin{figure}
\scalebox{1.25}{
<<label=carb-comp-test, fig=TRUE, echo=FALSE, width=10, height=7.5>>=
par(mar = c(3,20,4,3))
plot(carb.test)
@
}
