...
Basically, \code{pathview} maps and renders data provided by \code{annotated} on pathway graphs.
...
%TODO 
%explain the wrapper
To provide more convenient functionality for mapping PM-data on KEGG-pathway maps, we show a wrapper for the \code{pathview}-function. 
...
%TODO
% explain the two types of maps in KEGG
...
str(test.pdf)
@
% explain the content of test.pdf
...
Map of galactose metabolisms in the native KEGG representation available in xxx.  
}
\end{figure}
