...
i.e. the higher the value of the coefficient the more important is the variable.\footnote{This at least holds true for variables that have comparable standard deviations or standardised coefficients.}
...
\rcscom{(Currently we use ONE random effect for both technical and biological
  replicates; this is appropriate as we currently do not really want to differentiate
...
\rcscom{More technical details will be added here if necessary but we are
still in the process of discussing the results.}
...
\caption{Selected pathways and substrate classes. (left) From the selection path
  plot one can deduce that the number of iterations was sufficiently large as all
...
\caption{Coefficients of stable variables (for the optimal boosting iteration).\label{fig:stabsel3}}
\end{figure}

\subsubsection{Evaluation of results}

In the next step we will augment the boosting approach with an \code{opm\_mcp} analysis for confirming whether the reactions on carbohydrate wells are indeed more strongly different between the two strains than those on other wells.
