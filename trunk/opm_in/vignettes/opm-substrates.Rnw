...
Now we need to extract and restructure the data. We use all available biological
and technical replicates and obtain the annotation from KEGG:
<<make_data_set>>=
...
## add an ID per biological / technical replicate for usage of brandom:
data$ID <- extract_columns(data, c("Plate.number", "Strain", "Experiment"),
  direct = TRUE, factor = TRUE)
## drop plate number
data$Plate.number <- NULL
## have a look at a subset of the resulting data set
data[sample(1:nrow(data), size = 5), c(1:7)]
@
...
the annotations that indicate, whether this well can be mapped to the
corresponding substrate or pathway. Note that a well can potentially be mapped
to multiple substrates or pathways.

In the next step, we want to model the differences of the measured values
between the two strains. Therefore we set up a model of the following form:
\begin{align*}
  y = & \beta_0 + \beta_1 \text{strain} + \beta_2 \text{experiment} + \beta_3
  \text{slot} + b_{\text{ID}} +\\
...
Additionally, we use an random effect for the replicate ($b_{\text{ID}}$) to
...
## re-define base-learner to have only 1 degrees of freedom and no additinal
## intercept term
bols1 <- function(a)
  bols(a, intercept = FALSE, df = 1)
bols2 <- function(a)
  bols(a, df = 2)
...
fm <- as.formula(paste("log(Value) ~ Strain + ",
  paste(paste(names(data)[-c(1, 4)]), collapse = "+"), "+",
  paste("bols(", paste(names(data)[-c(1, 4)],
      collapse = ", by = Strain, intercept = FALSE, df = 1) + bols("),
    ", by = Strain, intercept = FALSE, df = 1)")))
## estimate model
mod <- gamboost(fm, data = data, baselearner = bols1,
...
use cross-validation techniques \citep{Hofner:mboost:2014,Mayr:mstop:2012}. This can be easily done
using:
...
strains with respect to certain substrates. However, cross-validations tends to
select to many variables to enter the model (here: \Sexpr{prob}\% of the
...
\citep[for the per-family error rate, see][]{Dudoit2003}. In our example the
...
boosting iterations. This graphical display mainly helps to verify if the number
...
\caption{Selected pathways and substrate classes. From the stability selection
  ...
  All variables that cross the threshold value (horizontal gray line;
  $\hat{\pi}_{\text{thr}} = \Sexpr{stab$cutoff}$) at some point are termed % $
  ...
  frequencies $\hat{\pi} \geq \hat{\pi}_{\text{thr}}$, i.e., right of the gray
...
\caption{Coefficients of stable variables (in the optimal boosting
  iteration). Positve signs indicate an increase in strain
...
\rcsmark{moved here to get coli.anno data set... do we need this?} First, we
