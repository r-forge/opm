...
\section{Available plate information}\label{plate-information}
...
\section{Accessing substrate information}\label{substrate-information}
...
The mapping itself works by using information produced by \pkg{opm} for a colour coding of the nodes (here, representing the substrates) within those maps, as can be done similarly with several other types of ``OMICS'' data, like for example transcriptomics or proteomics data.
...
This is usually only possible for the ``Dunnett'' and ``Pairs'' type of contrasts if applied to the wells.
...
In the following example, first an \code{opm\_glht} object is generated from the \code{vaas\_4} exemplar object by performing a Dunnett-type multiple comparison among the first 15 wells against well A05 as control group.
...
The picture was obtained by running \code{opm\_mcp()} and then the plotting function for the resulting \code{opm\_glht} object.
See the main tutorial for details.
}
\end{figure}
\end{center}

Using the above generated \code{opm\_glht} object, the options modifying the output of \code{annotated} will be illustrated.
...
For integrating experimental data from other ``OMICS'' approaches (such as transcriptomics and proteomics), see the corresponding chapter in the \code{pathview} vignette for details. 
...
All other necessary steps (download of pathway graph data, parsing of this data file, integrating user-defined data into the pathway representation, and rendering of final graph) are automatically  conducted by \code{pathview}.
...
\item[org -] Organism-specific pathway map; ``org'' is a wild-card for the organism specific abbreviation composed of two to four letters
\end{itemize}
...
e.coli.type <- subset(vaas_et_al, list(Strain = "DSM30083T"))
@
...
% bring the arguments in the same order as in the argument list of pathview

<<label=pathview-wrapper>>=
opm_path <- function(cpd.data, gene.data = NULL,
    high = list(gene = "green4", cpd = "blue"), 
    mid = list(gene = "lightsteelblue1", cpd = "red"),
...
First we show the results for the two strain on separate maps in Figure~\ref{fig:pathview-native-sep-strains}.
...
\begin{figure}
 \centering
        ...
        \begin{subfigure}[b]{1.0\textwidth}
                ...
                \caption{PM data of \textit{Escherichia coli} type strain DSM 30083\textsuperscript{T} mapped on KEGG galactose pathway.}
                \label{fig:1b}
        \end{subfigure}
        \\
...
Note, that from the annotation objects, \code{anno.k12} or \code{anno.typ} respectively, comprehending 96 substrates, only 13 are represented in the shown pathway map in Figure~\ref{fig:pathview-native-sep-strains}.
As described in section~\ref{annotated-input}, for some substrates, like ``pH5'' or ``positive Control'' no KEGG ID is available, which results in NA in the vector.
Accordingly, those substrates are not mapable to pathway maps. 
Further not all substrates from one plate are represented in one single pathway, but only substrate groups are represented in pathway maps.
By using the option \code{how = ``value''}, \code{annotated} results a numeric matrix with substrate names as rownames and pathway identifiers as column names. 
Ones and zeros indicate, if a certain substrate (rows) is element of a given pathway (columns), or not. 
NAs indicate, that the substrate has no KEGG ID, as for example well A01 with substrate ``positive Control''.

However, by summing up the columns and sorting the resulting vector, the user gets a ranking of the columns (pathways) representing how many substrates. 

<<label=anno_sum_mat_col>>=
anno.k12.mat <- annotated(e.coli.k12, how = "value")
col.sums <- apply(anno.k12.mat, 2, sum, na.rm = TRUE)
col.sums.sort <- sort(col.sums, decreasing = TRUE)
# show the first ten entries
col.sums.sort[1:10]
@

Further, the user is easily able to identify the substrates, represented in a choosen pathway.
In the here shown example that are the substrates represented in pathway number 00052, which is galactose metabolism.
...
The user is also able to extract the positions of these substrate on the plate
<<>>=
e.subs.pos <- find_positions(e.subs, type = "Gen III")
e.subs.pos
@

Now it is straightforward to compute a multiple comparison between only the 13 substrates included in the Galactose pathway map, since vector \code{e.subs.pos} contains the positions of the substrates of interest and can serve as direct input in \code{opm\_mcp}. 
Our example now compares the type strain with K12; corresponding 95\% CIs for differences of means on the choosen substrates are shown in Figure~\ref{fig:e_coli_comp_ci}.
...
%x <- ci_plot(e.coli.sub[, , c(e.subs.pos)], 
...
The annotation vector for the differences of means can be obtained straightforwardly by simple application of \code{annotated} on the \code{opm\_glht} object. 
This directly facilitates the mapping of the differences between the two strains on the galactose pathway, as it is shown in Figure~\ref{fig:pathview-native}
...
Mapping of differences between \textit{E. coli} K12 and type strain DSM 30083\textsuperscript{T} Galactose pathway.  
}
\end{figure}
...
%TODO thank Barry for substrate and plate information
