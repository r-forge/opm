...
\acrodef{VIMs}{variable importance measures}
...
Using the ``download'' argument, if \ac{KEGG} drug or compound IDs have been selected, these can be downloaded from the \ac{KEGG} server if the \pkg{KEGGREST} is available and converted into customised objects.
...
The \code{ko} entries are converted to gene identifiers if organism-specific pathways maps are generated.
...
With the usual \code{cex}-argument the scaling of text and symbols can be tuned (when \code{kegg.native=FALSE} default is \code{cex=0.5}) and \code{text.width} it is possible to specify the length for text wrapping. 
In principle, the node size can be fine-tuned by \code{afactor}, however even the help page of function \code{pathview} emphasises that ``its effect is subtler than expected''.
Together with the automated rendering by the \code{graphviz} layout engine it might be difficult to result a satisfying map of the pathway of interest.
...
In the next coding example, we prepare a data matrix of differences between means and potential explanatory variables from \ac{KEGG}.
...
%TODO Lea: I've changed this to non-formula input to avoid make.names().
% This is now the full Gen-III plate except for the NAs.
...
varImpPlot(coli.comp.rf, main = NULL)
@
...
some literature:
\begin{itemize}
\item \citep{Wouter2012} - review of random forest properties that allow maximising the biological insights that can be extracted from complex OMICS-data using \ac{RF}
\item \citep{Boulesteix2012} - emphasis on applications to bioinformatics and computational biology, special attention to practical aspects such as (i) selection of parameters, (ii) available \ac{RF} implementations, and (iii) important pitfalls and biases of \ac{RF} and its \ac{VIMs}
