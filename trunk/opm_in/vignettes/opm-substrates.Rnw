...
\begin{figure}
 ...
 \caption{Respiratory data (mean maximum height, discretised) of \textit{Escherichia coli} strain K12 mapped on the \ac{KEGG} Galactose pathway. The aggregated and discretised measurement data are represented by according colours and mapped on the corresponding substrates (circles) in the graph.}
...
We can now depict these features within a ``Variable Importance Plot'', which shows the absolute values of the standardised regression coefficients $\beta$ (Figure~\ref{fig:var-imp-plot}).
\rcsmark{standardised coefficients!?!} The higher the value of the standardised absolute coefficient, the higher is the contribution of the corresponding variable to the prediction of the differences of strains, i.e. the more important is the variable.
...
Plotting the results leads to a representation of the stability paths, i.e. the inclusion frequency dependent on the number of boosting iterations (see Figure~\ref{fig:stabsel}). As an alternative to the stability paths we can plot the maximum inclusion frequency (see Figure~\ref{fig:var-imp-plot2}).
Higher inclusion frequencies indicate that the substrate is more important.
Furthermore, we see that only the top \Sexpr{length(selected(stab))} variables are stable and thus can be considered to play a substantial role.
...
For now, we will augment the boosting approach with an \code{opm\_mcp} analysis for confirming whether the reactions on carbohydrate wells are indeed more strongly different between the two strains than those on other wells.
