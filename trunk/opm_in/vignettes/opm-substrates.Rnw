...
Using \code{species}-argument it is possible to include the genome annotations available from the \ac{KEGG} homepage (see above for details).
...
We chose the annoation of strain \textit{E. coli} K-12 MG1655 from year 1997, which results in \code{species = "eco"}. 
In the corresponding Figure~\ref{fig:e_coli_kegg_anno} genes (boxes) without annotation information remain white without any labelling.
However, using \code{na.col} the user is free to highlight entries without annotation information with colours of choice.
  
 
<<label=pathview_e_coli_comp_map>>=
coli.map.k12.eco <- opm_path(cpd.data = anno.k12, species = "eco", 
  out.suffix = "k12.eco", pathway.id = "00052")
@
...
Respiratory data (mean maximum height) of \textit{Escherichia coli} strain K12 mapped on the \ac{KEGG} Galactose pathway including annotation information of strain \textit{E. coli} K-12 MG1655 from year 1997 available from \ac{KEGG}. Genes without annotation information remain just white without any labelling.}
\end{figure}

Using this type of visualisation users are enabled to detect metabolic steps for which no genes are annotated, but \ac{PM} data indicate metabolic activity.
