...
Please note that the underlying test seeks for differences and thus the results always have to be interpreted regarding the significance (and magnitude) of differences; ``insignificantly different'' does not mean ``significantly equal''!
...
Now we can build the contrast matrix needed for the specified computation of the multiple comparisons between the two strains for the two subsets of substrates.
...
contr.n.carb <- opm_mcp(coli.sub[, , noncarb.pos], linfct = c(Dunnett = 1),
  model = ~ J(Well + Strain), output = "contrast")
@  
