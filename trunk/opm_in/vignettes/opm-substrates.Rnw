...
Before starting, install the package \pkg{pathview} (not shown below) and then load it together with the \pkg{opm} package, into an \proglang{R} session. Please, make sure, that you this is done in the stated order, otherwise \pkg{pathview} does not work properly.
...
In conjunction with other packages, it is possible to visualize PM results directly in pre-existing pathway maps as, for example, delivered by the KEGG database.
...
Depending on availability of genome and gene-annotation information within KEGG about a certain organism, individual maps are allocateable.
The mapping itself is simply the introduction of PM-measurement data into those maps, as it can be done similarly with several other types of OMICS-data. For details, please see the description on the KEGG-homepage: (\url{http://www.genome.jp/kegg/}).

Here we will use the function \code{pathview} from the package of same name \citep{Luo2013}. 
This function downloads the stated pathway map and maps and renders user data into it.

The workflow starts either with on object of class \code{OPM}, \code{OPMS} or \code{OPMX} object containing the agregated values or the results of an \proglang{opm\_mcp} analysis. 
Here we start with the former situation, while the latter case will be discussed further below.
The first step is to bring the PM-results in a suitable format, which is a named vector created by \code{annotated}. 
...
The resulting vector basically contains the numeric values (selected parameter estimates or \code{opm_mcp} results, as explained below) as well as an annotation of the according substrates. 
With the \code{what}-argument, passed as eponymous argument to \code{substrate_info}, the user can indicate which kind of information should be used for the annotation.
\code{annotated} works directly on \code{OPMS}-objects containing aggregated data for single plates or bundles of plates. 
...
Further options allow for indication of categorical (\code{different}) and for the \code{opm_glht} method, binary outcomes (\code{smaller}, \code{larger} and \code{equal}).
