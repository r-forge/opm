...
## re-define base-learner to have only 1 degrees of freedom and no additional
## intercept term
bols1 <- function(a)
...
#  paste(names(data)[-c(1, 4)], collapse = "+"), "+",
#  paste("bols(", paste(names(data)[-c(1, 4)],
#      collapse = ", by = Strain, intercept = FALSE, df = 1) + bols("),
#    ", by = Strain, intercept = FALSE, df = 1)")))
fm <- as.formula(paste(c("log(Value) ~ Strain", names(data)[-c(1, 4)],
  sprintf("bols(%s, by = Strain, intercept = FALSE, df = 1)",
    names(data)[-c(1, 4)])), collapse = " + "))
## estimate model
