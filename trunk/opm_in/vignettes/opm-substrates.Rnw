...
The mapping of significant differences between the two strains on the Galactose pathway is shown in Figure~\ref{fig:pathview-native-sig-differences}.
...
% TODO LATER: add example with the same data and a threshold larger than 0
%   this should yield partially insignificant differences in another
%   mapping of the categorical output

\begin{figure}
\scalebox{1.225}{
\includegraphics[type=png,ext=.png,read=.png,page=1]{ko00052.cat-coli-comp}
}
\caption{\label{fig:pathview-native-sig-differences}
