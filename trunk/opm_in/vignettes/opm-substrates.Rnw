...
Please refer to the manual and Section~\ref{annotated-input} for an explanation of the usage of \code{annotated} with the argument \code{how = "data.frame"}.
...
The data frame has to be reduced to the non-\code{NA} rows, i.e. substrates that are not included in \ac{KEGG} have to be removed, before the \pkg{mboost} package \citep{mboost:pkg:2013} can be applied.


\subsection{Boosting as a tool to identify important substrates}

In the following example we use boosting to estimate a linear model in order to determine important substrates that describe the differences between the experimental settings.
Optimal stopping of the algorithm can be done via cross-validation.
For a detailed hands-on introduction to boosting methods using the package \pkg{mboost} we refer to \citep{Hofner:mboost:2012}.

<<install.package.mboost, echo = FALSE, results = hide>>=
if (!suppressPackageStartupMessages(require("mboostDevel")))
...
## load package mboost
## (currently the development version "mboostDevel" from r-forge is used)
suppressPackageStartupMessages(library("mboostDevel"))
## fit the boosting model with 1000 initial iterations
mod <- glmboost(Value ~ ., data = coli.anno,
  control = boost_control(mstop = 1000))
## obtain optimal stopping iteration via crossvalidation
cvr <- cvrisk(mod)
## set model to optimal stopping iteration
mod[mstop(cvr)]
@

If one uses cross-validation techniques to find the optimal stopping iteration, boosting intrinsically selects variables that best predict the outcome, i.e. here the differences between the two \textit{E. coli} strains with respect to certain substrates.
We can now depict these features within a ``Variable Importance Plot'', which shows the absolute values of the regression coefficients $\beta$ (Figure~\ref{fig:var-imp-plot}).
Higher values of the absolute coefficient tend to indicate bigger contributions of the corresponding variable to the prediction of the differences of strains,
i.e. the higher the value of the coefficient the more important is the variable.\footnote{This at least holds true for variables that have comparable standard deviations or standardized coefficients.}

\begin{figure}
  \begin{center}
\scalebox{0.55}{
<<coef-plot2, fig=TRUE, echo=FALSE, width = 6>>=
old.par <- par(mar = c(4, 8, 0.1, 0.1)) # adapt plotting settings
cf <- coef(mod)[-1]
plot(sort(abs(cf)), 1:length(cf),
  type = "n", yaxt = "n", xlim = c(0, max(abs(cf))),
  ylab = "", xlab = expression(paste("|", beta, "|")))
abline(h = 1:length(cf), lty = "dotted", col = "grey")
points(sort(abs(cf)), 1:length(cf), pch = 19)
axis(2, at = 1:length(cf), names(cf)[order(abs(cf))], las = 2)
par(old.par) # reset plotting settings
@
}
\caption{Absolute value of coefficients at the optimal stopping iteration
  determined via cross-validation (\code{mstop} = \Sexpr{mstop(cvr)}).
...
Stability selection \citep{Meinshausen:2010} is an additional screening tool that allows the user to determine which of the selected variables are stable ``stable'' variables, i.e. which are variables that always ``pop-up'' in random subsets of the data.
Additionally, stability selection has a built in error control (for the per-family error rate).
In our example the maximum acceptable error is set to 1.5, i.e. we accept at maximum 1.5 noise variables to be chosen by stability selection by chance. The cut-off value determines, which inclusion frequency is at least required to term a variable stable.
In our example this is chosen to be $0.6$, but other values in, say $(0.6, 0.9)$, are possible as well.
In general, stability selection is relatively insensitive to the choice of the cut-off.
<<mboost_stability_selection, results = hide, eval = false>>=
## first, increase model again
mod[1000]
## now use stability selection
stab <- stabsel(mod, cutoff = 0.6, PFER = 1.5, error.bound = "SS")
@

<<mboost_stability_selection_hidden, results = hide, eval = true, echo = false>>=
if ( !file.exists("stabsel.Rda") ) {
<<mboost_stability_selection>>
    save(mod, stab, file = "stabsel.Rda")
} else {
    load("stabsel.Rda")
}
@
...
%TODO LATER:
% \item \citep{Boccuto2013} - analysis of metabolic profiles of lymphoblastoid
%   cell lines from 137 patients with neurodevelopmental disorders with or
%   without \acs{ASD} and 78 normal individuals, using BIOLOG Phenotype
%   microarrays. Feature selection approaches identified Tryptophan-substrates -
%   cells from \ac{ASDs} patients are less capable in utilisation of
%   Tryptophan-substrates
%\item

Plotting the results leads to a representation of the stability paths, i.e. the
inclusion frequency dependent on the number of boosting iterations (see
Figure~\ref{fig:stabsel}). This graphical display mainly helps to verify if the
number of boosting iterations was sufficient. In this case, the selection paths
stay constant after a certain point. As an alternative to the stability paths we
can plot the maximum inclusion frequency (see Figure~\ref{fig:var-imp-plot2}).
Higher inclusion frequencies indicate that the substrate is more important.
Furthermore, we see that only the top \Sexpr{length(selected(stab))} variables
are stable and thus can be considered to play a substantial role.

\begin{center}
\begin{figure}
\scalebox{1.1}{
<<label=stabsel-plot, fig=TRUE, echo=FALSE, width=12>>=
old.par <- par(mar = c(5, 4, 4, 7)) # adapt plotting settings
plot(stab, main = "Stability selection paths")
par(old.par) # reset plotting settings
@
}
\caption{Inclusion probability of variables (over the number of boosting
  iterations). The grey horizontal line indicates the threshold above which
  variables are termed ``stable''.}\label{fig:stabsel}
\end{figure}
\end{center}
...
\begin{figure}
  \begin{center}
\scalebox{0.55}{
<<max-include-freq, fig=TRUE, echo=FALSE, width = 6>>=
old.par <- par(mar = c(4, 8, 0.1, 0.1)) # adapt plotting settings
## number of substrates to plot
ps <- 20
## remove intercept as its selection is not interesting
inc_freq <- stab$max[-1]
plot(tail(sort(inc_freq), ps), 1:ps,
  type = "n", yaxt = "n", xlim = c(0, 1),
  ylab = "", xlab = expression(hat(pi)))
abline(h = 1:ps, lty = "dotted", col = "grey")
points(tail(sort(inc_freq), ps), 1:ps, pch = 19)
axis(2, at = 1:ps, names(inc_freq)[tail(order(inc_freq), ps)], las = 2)
## add cutoff
abline(v = stab$cutoff, col = "grey")
par(old.par) # reset plotting settings
@
}
\caption{Maximum inclusion frequency $\hat{\pi}$ for the top 20
  variables as determined by stability selection. \label{fig:var-imp-plot2}}
\end{center}
\end{figure}

%For details on stability selection in the context of boosting see \rcsmark{DRAFT \acs{BH}}.

The results can be interpreted a follows:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%  Alternativer Ansatz mit Variation in den Daten:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

<<set_seed_again, echo = FALSE, results = hide>>=
## set seed (for reproducible results)
set.seed(1907)
@

\subsubsection{Using repeated measurements as input}

Using two biological replicates and all technical replicates thereof the
statistical model gets more complicated. First we need to extract and
...
data <- opm_mcp(coli.sub1, ~ Strain + Experiment +
  Slot + 'Plate number', output = "data", in.parens = FALSE)

y <- annotated(coli.sub, how = "data.frame")
## add everything but the value
data[, colnames(y)[-1]] <- y[as.character(data$Well), -1]
data <- data[complete.cases(data), ]
data <- data[, !(names(data) %in% c("Well", "Parameter"))]

## restructure plate number to ID for usage of brandom:
data$ID <- as.numeric(as.character(data$Plate.number)) +
  10 * (as.numeric(data$Strain) - 1) +
  20 * (as.numeric(data$Experiment) - 1)
data$ID <- as.factor(data$ID)
data$Plate.number <- NULL
@

Now we can set up the model and use stability selection to extract the substrate
groups and pathways that best explain the difference between strains. Be
carefull as this takes a while.
<<model_code, eval = false>>=
fm <- as.formula(paste("Value ~ Strain + ",
  paste(paste(names(data)[-c(1, 4)]), collapse = "+"), "+",
  paste("bols(", paste(names(data)[-c(1, 4)],
      collapse = ", by = Strain, df = 2) + bols("),
    ", by = Strain, df = 2)")))

## re-define base-learner to have only 2 degrees of freedom
bols2 <- function(a)
  bols(a, df = 2)

## fit offset model with main effects only (and use very many iterations):
options(contrasts = c("contr.treatment", "contr.poly"))
offsetmod <- gamboost(Value ~ ., data = data, baselearner = bols2,
  control = boost_control(mstop = 6000))

## now start from the offset model and add interaction effects (which
## represent differences between strains).
options(contrasts = c("contr.sum", "contr.poly"))
mod2 <- gamboost(fm, data = data, baselearner = bols2,
...
stab2 <- stabsel(mod2, cutoff = 0.6, PFER = 1.5, error.bound = "SS")
@

<<model, echo = false, results = hide>>=
if ( !file.exists("pathway_model.Rda") ) {
<<model_code>>
  save(offsetmod, mod2, stab2, file = "pathway_model.Rda")
} else {
  load("pathway_model.Rda")
}
@

The model we used first fits all main effects, i.e. differences in the maximum
curve height with respect to different substrate groups or pathway maps while
neglecting possible differences in substrate/map effects between strains.
Additionally, we specify effects for strain, experiment and slot to account for
overall differences between strains, experiments and slots. To account for
repeated measurements we include a random effects term in the model
\rcscom{(currently we use ONE random effect for both technical and biological
  replicates; this is ok as we currently do not really want to differentiate
  between the two types of variability;)}. In a second step we allow for
interactions between strain and the substrate/map effects and check if any are
present. We use stability selection to determine which of the (interaction)
effects are selected (while controlling the per-family error rate).
\rcscom{More technical details if necessary but first we should discuss the
  results}

Plotting the results
<<model_pathways_code, eval = false>>=
old.par <- par(mar = c(4, 0.1, 0.1, 15), mfrow = c(1, 2))# adapt plotting settings
plot(stab2, main = "")
par(mar = c(4, 15, 0.1, 0.1))
## number of substrates to plot
ps <- 20
inc_freq <- stab2$max
plot(tail(sort(inc_freq), ps), 1:ps,
...
abline(h = 1:ps, lty = "dotted", col = "grey")
points(tail(sort(inc_freq), ps), 1:ps, pch = 19)
nms <- gsub(", ", " by ", variable.names(mod2))
axis(2, at = 1:ps, nms[tail(order(inc_freq), ps)], las = 2)
## add cutoff
abline(v = stab2$cutoff, col = "grey")
par(old.par) # reset plotting settings
@
leads to a similar but more complex structure (see Figure~\ref{fig:stabsel2}).
%\rcsdel{The forthcoming version of this vignette will demonstrate feature selection from the curve parameters themselves. We will show how to easily generate matrices containing estimated both traditional \pkg{opm} metadata and novel \pkg{opm} substrate metainformation as (potential) explanatory variables.}
...
\caption{Selected pathways and substrate classes. (left) From the selection path
  plot one can deduce that the number of iterations was suffiently large as all
  selection paths stop to increase after 100 iterations. (right) Maximum
  inclusion frequency $\hat{\pi}$ for the top 20 variables as determined by
  stability selection.\label{fig:stabsel2}}
\end{figure}

Now we can have a look at the regression coefficients of the selected variables.
Therefor we first determine the optimal stopping iteration of the boosting algorithm:
<<cvrisk_mod2_code, eval=false>>=
## first increase the model
mod2[1000]
cvr2 <- cvrisk(mod2)
mod2[mstop(cvr2)]
@

<<cvrisk_mod2, echo = false, results = hide>>=
if ( !file.exists("cvrisk_mod2.Rda") ) {
<<cvrisk_mod2_code>>
  save(cvr, file = "cvrisk_mod2.Rda")
} else {
  load("cvrisk_mod2.Rda")
}
@

In the next step we extract the coefficients of the stable variables
<<coefficients_code, eval = false>>=
cf <- coef(mod2, which = selected(stab2))
## use only the second coefficient, the first coefficient of each base-learner
## is the intercept which isn't of interest here.
beta <- sapply(cf, function(x) x[2])
old.par <- par(mar = c(4, 15, 0.1, 0.1))# adapt plotting settings
plot(sort(beta), 1:length(beta),
     type = "n", yaxt = "n",
     ylab = "", xlab = expression(hat(beta)))

abline(h = 1:length(beta), lty = "dotted", col = "grey")
points(sort(beta), 1:length(beta), pch = 19)
nms <- gsub(", ", " by ", variable.names(mod2, which = selected(stab2)))
...
\caption{Coefficients of stable variables (for the optimal boosting iteration).\label{fig:stabsel2}}
\end{figure}

\subsubsection{Evaluation of results}

\rcsmark{For now,} we will augment the boosting approach with an \code{opm\_mcp} analysis for confirming whether the reactions on carbohydrate wells are indeed more strongly different between the two strains than those on other wells.
