...
opm_path <- function(cpd.data, gene.data = NULL,
  ...
  pathview(cpd.data = cpd.data, gene.data = gene.data,
    high = high, mid = mid, low = low, pathway.id = pathway.id, 
...
Note particularly the substrates Raffinose, Stachyose and Sucrose (in the middle of the map), which exhibit large respiratory differences, while Sorbitol (on the very left in the map) indicate only small respiratory difference between the two strain.
...
\begin{figure}
 \centering
        \begin{subfigure}[b]{1.0\textwidth}
                ...
                \caption{Respiratory data (mean maximum height) of \textit{Escherichia coli} strain K12 mapped on KEGG galactose pathway.}
                \label{fig:1a}
        \end{subfigure}
        \\
        \vspace{20pt}
        \begin{subfigure}[b]{1.0\textwidth}
                ...
                \caption{Respiratory data (mean maximum height) of \textit{Escherichia coli} type strain DSM 30083\textsuperscript{T} mapped on KEGG galactose pathway.}
                \label{fig:1b}
        \end{subfigure}
        \caption{
        ...
        The quantitative measurement data are represented by according colours and maped on the corresponding substrates (circles) in the graph. 
        }
\end{figure}

The \code{opm_path} function downloads an .xml file from KEGG (corresponding to the choosen pathway map, see below for details), maps the provided input data in form of the annotation vector object and renders the output graphic (see the \code{pathview} vignette for technical details).
Using the default settings, the output resulting from \code{opm\_path} is a raster image in PNG format, which is stored in the current working directory and shown in Figure~\ref{fig:pathview-native-sep-strains}.
...
Genes (boxes) are annotated with KEGG onthology numbers (set by choosing \code{species = "ko"}), where available or, alternatively, with EC numbers. 
Note that the \code{species} arguments offers the possibility to use species-specific genome information available in the KEGG directory. 
Check the KEGG homepage for the current list of available genome information and corresponding letter codes (\url{http://www.genome.jp/kegg/}).
The substrates (circles) in the maps get standard compound names, which are automatically retrieved from CHEMBL database using the compound IDs. 

Further, the function generates a list containing two data frames, which are stored in the objects \code{e.coli.map.k12} and \code{e.coli.map.type}, respectively.

<<label=pathview-wrapper-outob>>=
str(e.coli.map.k12)
@
 
The data frame \code{plot.data.gene} contains the data for mapping the genes and \code{plot.data.cpd} analogously is the storage for compound related data.

Note that from the annotation objects \code{anno.k12} or \code{anno.type}, respectively, comprehending 96 substrates, only 13 are represented in the shown pathway map in Figure~\ref{fig:pathview-native-sep-strains}.
As described in section~\ref{annotated-input}, for some substrates, like ``pH 5'' or ``Positive Control'' no KEGG ID is available, which results in NA in the vector.

Accordingly, those substrates cannot be marked within pathway maps. 
...
By using the option \code{how = "value"}, \code{annotated} results a numeric matrix with substrate names as row names and pathway identifiers as column names. 
...
The user is also able to extract the positions of these substrates by using \code{type} argument: 
...
First, we show the maximum-height values from the thirteen substrates represented in the Galactose pathway map (number 00052) in Figure~\ref{fig:e_coli_single_ci}, to get an idea about the sizes of their differences.
Note that the vector \code{e.subs.pos} contains the positions of the substrates of interest as a character string and thus can serve as direct input in the squared brackets subsetting \code{OPMX} objects.
...
\begin{figure}
\scalebox{1.1}{
<<label=e_coli_single_ci_figure, fig=TRUE, echo=FALSE, height=16, width=10>>=
ci_plot(e.coli.sub[, , c(e.subs.pos)], 
  as.labels = list("Species", "Strain"), subset = "A", 
  x = "bottomright", 
  draw.legend = FALSE, crr = 1.33)
@
}
\caption{
...
The annotation vector for the differences of means can be obtained by simple application of \code{annotated} on the \code{opm\_glht} object. 
The direct mapping of these differences between the two strains on the galactose pathway is facilitated as it is shown in Figure~\ref{fig:pathview-native}.
