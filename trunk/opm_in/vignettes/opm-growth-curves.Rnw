...
% Do NOT REPEAT that stuff here -- here we must concentrate on the format and its manipulation, see nect entry.
...
%TODO Lea: briefly explain what is going on here within the call to reshape().
