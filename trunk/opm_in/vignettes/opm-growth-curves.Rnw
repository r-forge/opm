...
The interpretations given in the figure captions should be assessed using \code{opm\_mcp}.
Restrict yourself to the fresh mass and the TECAN data and to one call to \code{opm\_mcp}, and one plot of the resulting \code{glht} object, per data set.
Provide some brief explanations on the function arguments specific to these two data sets.
Omit all issues not specific to growth data.
A final sentence should relate the \code{opm\_mcp} results to the statements in the figure captions, i.e. the following two:
``It also indicates that the stresses impair growth but that the genetically modified cells grow better then the wild type under moderate stress conditions.''
and
``The plot indicates that one of the strains outgrows the other unless high concentrations of Glucose are applied.''
Omit anything else.
