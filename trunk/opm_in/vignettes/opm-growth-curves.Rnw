...
<<label=agg-opm-spline, echo=TRUE>>=
...
<<label=plot-opm-spline-dummy, eval=FALSE, echo=TRUE>>=
...
<<label=select-splines, echo=TRUE>>=
tecan <- sm.tecan
...
This can be confirmed with the \code{opm\_mcp} analysis, as significant differences between the two strains only occur for moderate Glucose concentrations (wells ``A02'' and ``A03'') but neither in the control treatment nor for high concentrations of the sugar.
