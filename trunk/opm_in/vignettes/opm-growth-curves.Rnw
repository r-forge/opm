...
Special care is need to input such data, but as we know that two measurements of a 96-well-plate are contained, we can actually use the \code{read.table} function that comes with \proglang{R}.
