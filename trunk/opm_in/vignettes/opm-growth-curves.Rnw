...
Thus directly using the ``horizontal'' input format of \code{opmx} is \textit{recommended} for manually entering data.
...
Of course, registering of plates can always be done within the \proglang{R} file that makes use of these plates, too.
...
Here two arguments of the same name are passed to the function for registering the full name and the well mapping in a single call:
...
Because of the repetitions in the substrate names (note the numbering, which is necessary here to generate unique substrate names, and later on important to split the plate), the texts can also be generated with less code:
...
After data recording, the Magellan\textsuperscript{TM} software generates such as file \textit{via} the ``save as \code{.asc}'' option in the ``edit'' menu.
...
See the manual for further details on the usage of this argument.
...
An example file for an EcoPlate\texttrademark \ analysis not conducted with an  but measured with a PerkinElmer, Inc., plate reader is included in \pkg{opm}.
...
Moreover, the comma has been used decimal separator (the data were probably measured with the language set to German).
...
So the question arises which spline estimation is optimal for such measurements, because all spline-fitting procedures have been optimised for estimating parameters from \ac{PM} data.
  ...
  for (param in rev(param_names())) {
    ...
    for (i in seq_along(y)[-1L])
      for (j in seq.int(1L, i - 1L)) {
        plot(y[[i]], y[[j]], pch = 19, col = "darkgrey",
...
After these preparations the methods can be visually compared as follows.
...
Note also that neither \ac{AUC} nor \ac{A} suffer from that problem.
Selecting a \ac{A} value that makes sense is hampered by an increase followed by a decrease again, as also visible in Figure~\ref{fig:growth_xy_plot}.
Thin-plate splines usually pick the actual maximum within a smoothed curve, which is biologically reasonable.
...
The plot in Figure~\ref{fig:growth_xy_plot} indicated that one of the strains outgrows the other unless high concentrations of Glucose are applied.
