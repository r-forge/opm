...
The following (informal) comparison, however, shows little differences between the methods.
...
plot_param_diff <- function(...) {
  ...
  for (param in param_names()) {
    ...
    for (name in names(y)[-1]) {
      plot(y[[1]], y[[name]], xlab = names(y)[1], ylab = name,
        main = param, pch = 19, col = "darkgrey")
      abline(line(y[[1]], y[[name]]), lty = "dashed")
    }
  }
  invisible(NULL)
}
...
sm.tecan <- do_aggr(tecan, method = "splines", boot = 0,
  options = set_spline_options(type = "smooth.spline"))
tp.tecan <- do_aggr(tecan, method = "splines", boot = 0,
  options = set_spline_options(type = "tp.spline"))
p.tecan <- do_aggr(tecan, method = "splines", boot = 0,
...
plot_param_diff(Sm = sm.tecan, Tp = tp.tecan, P = p.tecan)
par(old.par)
@
...
Correlation of curve parameters estimated using different spline-fitting options.
There are obviously quite high correlations for all parameters, hence spline fitting appears to be robust even when dealing with comparatively few data points.
Negative \ac{lambda} values can occur in the case of negative reactions.
}
\end{figure}

Apparently the distinct spline-fitting methods yield approximately the same estimates for all parameters.
Thus even in the case of comparatively few data points it seems that spline fitting as implemented in \pkg{opm} can robustly be applied.
For the forthcoming analyses we restrict ourselves to the results obtained with smoothing splines.

<<label=select-splines>>=
tecan <- sm.tecan
rm(tp.tecan, p.tecan, sm.tecan) # tidy up
potato.fm <- do_aggr(potato.fm, method = "splines", boot = 0,
  options = set_spline_options(type = "smooth.spline"))
potato.dm <- do_aggr(potato.dm, method = "splines", boot = 0,
  options = set_spline_options(type = "smooth.spline"))
@
