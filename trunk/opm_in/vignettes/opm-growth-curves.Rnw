...
So the ``plate position'' is also virtual, but just acts as an identifier of the plate.
...
\rcscom{[Lea, please check whether this is the correct interval argument for these data.]}
...
xy_plot(potato.fm, theor.max = FALSE, rcr = 1, include = "Genotype",
  main = list(in.parens = FALSE), ylab = "Mass [g]", neg.ctrl = FALSE)
@
