...
Using the dedicated \code{plot} method, the results can be visualised as demonstrated in Figure~\ref{fig:dm-mcp} and Figure~\ref{fig:fm-mcp}.

\begin{figure}
\centering
<<label=plot-fm-mcp, echo=FALSE, fig=TRUE>>=
...
\caption{
\label{fig:fm-mcp}
Point estimates and 95\% confidence intervals in a ``Pairs''-type comparison of group means for the fresh-mass exemplar object.
...
\begin{figure}
\centering
<<label=plot-dm-mcp, echo=FALSE, fig=TRUE>>=
old.mar <- par(mar = c(3, 15, 3, 2))
plot(dm.mcp)
par(old.mar)
@
\caption{
\label{fig:dm-mcp}
Point estimates and 95\% confidence intervals in a ``Pairs''-type comparison of group means for the dry-mass exemplar object.
Compare the outcome with Figure~\ref{fig:potato_xy_plot} and see the main text for an interpretation.
}
\end{figure}

Results obtained with \code{opm\_mcp} confirm the suspicion from Figure~\ref{fig:potato_xy_plot} that the stresses impair growth but that, when measured using fresh mass (Figure~\ref{fig:fm-mcp}), the genetically modified cells grow better then the wild type under moderate stress conditions.
In contrast, the wild type outgrows the genetically modified lineages under non-stress (control) conditions.
In the dry-mass measurements (Figure~\ref{fig:fm-mcp}) some differences are less apparent.
...
The TECAN can be analysed in an analogous fashion.
