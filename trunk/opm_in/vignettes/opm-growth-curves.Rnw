...
The data frames passed to \code{opmx} contain all substrate information (in the ``Treatment'' column).
...
Finally, it will be shown how to use the \code{split} function to split the object into one object per repetition.
...
Here two argument of the same name are passed to the function for registering the full name and the well map in a single call:
...
However, manually entering the well map is error prone and not efficient when dealing with data sets containing more than a few wells.
...
This file was output by a \rcsmark{[Lea please enter the name]} instrument as distributed by the TECAN corporation.
After recording the data, the \rcsmark{[Lea please enter the name]} software provided by TECAN generated such as file \textit{via} \rcsmark{[Lea please enter the procedure]}.
...
tecan <- opmx(tecan, "rectangular", plate.type = "growth", position = 1,
  interval = 1)
tecan
@

Note that we have to refer to the previously registered plate type, ``growth''.
The \code{position} argument is important for identifying each plate if several plates of this plate type are dealt with.
The optional \code{interval} argument provides the time interval between two consecutive measurements.
Ideally, it is provided in hours.
Time series with irregular intervals can be entered with the same argument (by directly providing each time point).

\rcscom{
TODO: Lea, please check whether this is the correct interval argument for these data.
}
...
\rcscom{
TODO: Lea, please enter the correct strain designations in the code snippet for assigning metadata.
}
...
For instance, the expected maximum for \ac{PM} data can seldom be used for delimiting the y axes, and the data are not measured in OmniLog\textregistered \ units:

<<label=potato-growth-dummy, eval=FALSE>>=
xy_plot(potato.fm, theor.max = FALSE, rcr = 1, include = "Genotype",
  main = list(in.parens = FALSE), ylab = "Mass [g]")
@
...
See the main tutorial for details in this kind of plotting.
}
\end{figure}

The result is shown in Figure~\ref{fig:potato_xy_plot}.
The TECAN data can be visualised in the same way as the potato data, yielding Figure~\ref{fig:growth_xy_plot}.
...
xy_plot(tecan, theor.max = FALSE, include = "Strain",
  main = list(in.parens = FALSE), ylab = "Value", rm.num = TRUE)
@
...
\rcscom{
  TODO: Lea, please enter an appropriate y-axis description for the E. coli plot.
}
...
\rcscom{
TODO: Lea, please make recommendations regarding the spline method to be used with these date.
