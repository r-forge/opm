...
See Section~\ref{tutorial-methods aggregating} in the main tutorial for details on this.

<<label=agg-dummy, eval=FALSE, echo=TRUE>>=
potato.agg.dm <- do_aggr(potato.dm, method = "opm-fast", boot = 0,
  options = list(as.pe = "pe"))
@

for potato: only FM-data, due to issues in the experimental setup,

show test of spline-methods

for coli-data: show spline-method-testing and select
