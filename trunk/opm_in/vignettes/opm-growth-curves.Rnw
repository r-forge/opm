...
pe.file <- grep("perkin_elmer", pe.file, ignore.case = TRUE, value = TRUE)
...
pe <- read.table(pe.file, strip.white = TRUE, sep = "\t",
...
# convert to OPMS
pe <- opmx(pe, "horizontal", full.name = c("custom:myeco" = "Ecoplate"),
...
pe.sep <- split(pe)
heat_map(pe.sep, NULL, extract.args = list(rm.num = TRUE))
@
...
library("gridExtra")
plot.fm <- xy_plot(potato.fm, theor.max = FALSE, rcr = 1,
  include = "Genotype", main = list(in.parens = FALSE),
  ylab = "Fresh cell mass [mg]", neg.ctrl = FALSE)
plot.dm <- xy_plot(potato.dm, theor.max = FALSE, rcr = 1,
  include = "Genotype", main = list(in.parens = FALSE),
...
The main difference with respect to user-entered growth-curve data is that these may contain much fewer time points than \ac{PM} measurements.
...
tp.tecan <- do_aggr(tecan, method = "splines",
  options = set_spline_options(type = "tp.spline"))
p.tecan <- do_aggr(tecan, method = "splines",
  options = set_spline_options(type = "p.spline"))
sm.tecan <- do_aggr(tecan, method = "splines",
...
potato.fm <- do_aggr(potato.fm, method = "splines",
  options = set_spline_options(type = "smooth.spline"))
potato.dm <- do_aggr(potato.dm, method = "splines",
  options = set_spline_options(type = "smooth.spline"))
@
...
<<label=potato-mcp>>=
dm.mcp <- opm_mcp(potato.dm, ~ J(Well, Genotype), m.type = "aov",
  linfct = c(Pairs.Well = 1), max = 7, in.parens = FALSE)
fm.mcp <- opm_mcp(potato.fm, ~ J(Well, Genotype), m.type = "aov",
  linfct = c(Pairs.Well = 1), max = 7, in.parens = FALSE)
@
...
<<label=tecan-mcp>>=
tecan.mcp <- opm_mcp(tecan, ~ J(Well, Strain), m.type = "aov",
...
Cordial thanks are addressed to Lea A.I. \German{Vaas} (\ac{DSMZ}) for providing the potato growth measurements and background information on these data, as well as for clarifying some technical issues related to the TECAN data.
This tutorial did benefit from discussions on the application of spline-fitting with Benjamin \German{Hofner} (\ac{FAU}) and Johannes \German{Sikorski} (\ac{DSMZ}).
