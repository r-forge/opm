...
The output of that reader is regular but contains a tab-separated part as well as bottom part with the same measurement in a different layout and technical information on the run:
...
Before the conversion to an \code{OPMS} objects, we register the plate type as a user-defined plate type even though EcoPlate\texttrademark \ is identical to it.
...
The next steps is typical for all EcoPlate\texttrademark \ data.
As this plate type contain repetitions of substrates, it is often used to measure distinct organisms or experimental conditions, or distinct replicates of the same organism or experimental condition, in one run.
