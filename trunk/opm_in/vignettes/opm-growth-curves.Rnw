...
This can be circumvented by placing code for loading \pkg{opm} and for registering the plates of interest in an ``.Rprofile'' file or in the ``Rprofile.site'' file.
...
%In terms of dry mass the wild type is
}
\end{figure}
...
For keeping this vignette comprehensive, only the subset containing the dry mass data will be analysed.

<<label=agg-opm-fast, echo=TRUE>>=
potato.agg.dm <- do_aggr(potato.dm, method = "opm-fast", boot = 0,
  options = list(as.pe = "pe"))
@

However, the probably more precise option is to use a spline estimation.

<<label=agg-opm-spline, echo=TRUE>>=
sm <-set_spline_options(type = "smooth.spline")
tp <-set_spline_options(type = "smooth.spline")
p <-set_spline_options(type = "smooth.spline")

sm.tecan <- do_aggr(tecan, method = "splines", options = sm)
tp.tecan <- do_aggr(tecan, method = "splines", options = tp)
p.tecan <- do_aggr(tecan, method = "splines", options = p)
...
plot_param_diff <- function(...) {
  ...
  for (param in param_names()) {
    ...
    for (name in names(y)[-1L])
      plot(y[[1L]], y[[name]], xlab = names(y)[1L], ylab = name, main = param)
  }
  invisible(NULL)
}
@
...
\begin{figure}
\centering
<<label=plot-opm-spline, echo=TRUE, fig=TRUE>>=
par(mfrow = c(4, 2))
...
\end{figure}
\rcscom{TODO: Lea, please write a better caption about the correlation of curve parameters estimated using different spline-options}
% remaining code
%
...
\rcscom{
TODO: Lea, write for coli-data: show spline-method-testing and select
}
%
%# tecan-daten
%tecan.agg <- do_aggr(tecan, method = "opm-fast", boot = 0,
%  options = list(as.pe = "pe"))
%
%
%##
