...
#'   The set of existing types of contrast is extended with the contrast type
#'   \sQuote{Pairs}. Here all pair-wise comparison concerning the first entry in
#'   \code{model} are computed. Alternatively, the factor whose levels should
...
#'   set of contrasts. In analogy to \code{Pairs}, the name of the level,
#'   separated from the string \sQuote{Dunnett} by any sign, can be stated. See
#'   examples below and in the vignettes and further \code{contrMat} from the
#'   \pkg{multcomp} package.
#'
#'   In situations where metadata have non-syntactic names, special signs are
...
#'   between the \code{linfct} name and the metadata name must not be a dot.
#'
#'   \item An object inheriting from the \code{AsIs} as created by \code{I} from
#'   the \pkg{base} package. Such objects, irrespective of their class, will be
#'   converted to an argument list for and then passed to \code{mcp} from the
#'   \pkg{multcomp} package.
#'
...
#'
#'   If the \code{output} argument is set to \kbd{linfct}, the function
...
#'   to use in model fitting: \kbd{glm}, \kbd{aov} or \kbd{lm}. See the
#'   eponymous functions in the \pkg{stats} package for details.
#'
...
#'     \item{mcp}{The default. An object of class \code{glht} as the result of
...
#'     is an object of class \code{mcp} or convertible to such an object,
