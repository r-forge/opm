...
#' potentially multiple plate types. Regarding the name: the \acronym{OPMS}
#' stands for \sQuote{multiple}. \acronym{MOPMX} objects are generated by
#' \code{\link{read_opm}} and \code{\link{opms}} if the \sQuote{grp} option is
#' used. They currently contain few methods of their own and behave like lists.
#'
#' @examples
...
setClass(MOPMX,
  ...
  prototype = list(),
  validity = function(object) {
    if (all(vapply(object, is, NA, OPMX)))
      TRUE
    else
...
setMethod("initialize", MOPMX, function(.Object, ...) {
  .Object <- callNextMethod()
  if (any(bad <- vapply(.Object@.Data, is.null, NA)))
    .Object@.Data <- .Object@.Data[!bad]
  .Object
}, sealed = SEALED)
